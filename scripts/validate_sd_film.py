...
def validate_state_routing(root: Path, as_json: bool = False, emit_report: bool = True) -> int:
    ...
    for marker in (
        ...
        "Fallback Source",
        "Latest Successfully Loaded Current Skill Definition",
        "实际重新读取当前权威`SKILL.md`",
        "Explicit Re-entry Command",
        "Runtime Skill Reload + Workflow Re-entry / Re-route",
    ...
    if "Runtime Skill Reload + Workflow Re-entry / Re-route" not in skill_text:
        errors.append("SKILL.md Runtime Reload Entry does not require Workflow Re-entry")

    progression_path = root / "rules" / "progression_rules.md"
    progression_text = read_text(progression_path) if progression_path.is_file() else ""
    if "普通的“继续”“下一步”不是重载触发词" not in runtime_text or "纯推进命令" not in progression_text:
        errors.append("Runtime Re-entry must preserve the plain-continue progression boundary")

    guide_path = root / "USER_GUIDE.md"
    guide_text = read_text(guide_path) if guide_path.is_file() else ""
...
def validate_ref_sketch_evidence(path: Path, skill_root: Path, as_json: bool = False) -> int:
    ...
    if evidence.get("neutral_mannequin_representation") is not True:
        errors.append("Character Appearance Leakage Check requires neutral_mannequin_representation=true")
    if evidence.get("character_appearance_leakage") is not False:
        errors.append("FAIL = Character Appearance Leakage / Identity Contamination")
...
def validate_skill(root: Path, as_json: bool = False) -> int:
    ...
    integration_checks = {
        ...
        "templates/08_shot_design_prompt.md": ("Professional Detailed Shot Script Template", "镜号", "TC IN", "TC OUT", "时长(s)", "景别", "焦段", "场景 / 美术", "画面内容 / 构图", "人物动作", "摄影机 / 镜头", "摄影参数", "镜头调度", "光线 / 色彩", "画面特效 / 转场", "台词 / 旁白 / 口播", "同期声音设计", "AI制作备注", "素材 / 资产", "摄影机运动", "人物调度", "镜头结束状态", "Pre-action / In-action / Post-action", "Primary Performer", "只写“紧张、从容", "前景、中景、背景", "Start Boundary", "End-Frame Constraint", "Next-Shot Handoff", "Director Decision Layer必须读取已经完成的专业分镜表", "templates/10_video_prompt.md", "Artifact Revision", "Source Script Labels"),
        ...
        "templates/20_clip_plan.md": ("# Clip Plan", "Source Detailed Shot Design Artifact / Portable Checkpoint", "Source Detailed Shot Design Status", "Source Detailed Shot Design Revision", "Unit Rule", "Namespace Rule", "包含 Shot", "起始状态", "连续动作", "Performance / Emotion Check", "Arc Endpoint", "Intentional Hold", "Primary Performer", "Visual Blocking Risk Pre-Assessment", "Visual Anchor State / Blocking Signature", "Clip Movement Plan", "主导镜头语言", "视觉高潮镜头", "最克制镜头", "重复规避", "Seedance复杂度控制", "空间关系", "道具连续性", "结尾状态", "Clip End-State Record / Next-Clip Carryover", "Character State", "Continuity Risks", "Reference Selection / Routing", "Tail Frame Required = YES / NO", "待用户提供/待上传", "Visual Input Eligibility", "NOT ELIGIBLE", "每个Clip只生成一条连续Prompt"),
        ...
        "knowledge/performance/micro_expression.md": ("Performance Progression Engine", "Action-phase Performance Rule", "Pre-action → In-action → Post-action", "PL1 / PL2 / PL3", "1—2个载体", "2—4个按先后发生的载体", "不是固定六阶段时间轴", "Attention Shift / Appraisal", "Expression Is Conditional", "微表情是短暂泄漏"),
        ...
        "knowledge/performance/emotion_dynamics.md": ("Emotion Is A Process", "Performance Arc Map", "Pre-action State", "In-action Change", "Post-action Residue", "Arc Endpoint", "Intentional Hold", "Mixed Emotion And Subtext", "Suppression And Leakage", "Performance Continuity Ledger", "Stable Downgrade"),
        "knowledge/performance/group_reaction.md": ("Relative Performance Hierarchy", "Primary Performer", "Counterpoint / Secondary Reactor", "Listener / Witness", "Background Holder", "Reaction Cascade"),
        ...
        "references/project_state_contract.md": ("Canonical Portable State Schema", "State Status: NOT_STARTED", "Next Workflow: 01_project_setup_workflow.md", "## State Control", "State Status", "Last Successful Checkpoint", "Review Result", "Revision ID", "Source Material", "Adaptation Draft", "Optimized Proposal", "Production-Locked"),
