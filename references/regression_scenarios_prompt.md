...
### R15-H Positive Specification And Negative Placement

输入：`岩井俊二式潮湿夏日青春气质；禁止夸张微笑、甜宠式表演、广告摆拍、MV慢动作与炫技运镜。`

PASS：`主风格：`可以保留并展开`岩井俊二式潮湿夏日青春气质`，但以柔散自然窗光、低饱和灰绿/米白、安静观察式摄影、克制含蓄表演、细小停顿/视线/手部动作与简洁自然镜头调度等当前Clip必要的正向carriers执行；不在正文重复原负向清单。仍属当前高风险、正向描述难完全锁定的残余错误按Negative Placement**归位**：与该阶段内容相关的写成所属字段内的最短约束或持续正向状态（字幕控制在字幕出现位置，音频控制在需控制音频的段落附近），只有真正跨阶段、无法下放到任何单一阶段的部分才经过合并压缩后进入末尾唯一`反向提示词：`。

FAIL：在`主风格 / 人物一致性 / 画面描述 / 人物动作与情绪`继续散布通用“禁止 / 不要 / 避免”清单；把原句完整复制到多个字段；把某个阶段的内容约束抽离所属字段搬进末尾段，导致分镜指代不清；删除风格标签但没有建立执行carriers；末尾反向段与正文同义重复或比动作/空间/镜头正文更长更抢权重。
...
PASS：在该分镜的`起始状态 / 人物动作与情绪 / 道具状态 / 镜头结尾状态`中适用位置保留最小、指代清楚的连续性约束——这是内容相关约束的**默认位置**，优先写成`左手持续握住伞柄，整个动作链与结尾状态均保持左手持有`；不把它扩展成局部通用负向清单。真正跨阶段的高风险项仍只在末尾唯一反向段收束。

FAIL：把持伞手约束整块搬进末尾反向段，导致具体动作链指代不清；在每个字段重复“不能换手”；把雨伞约束扩展成长串通用负向词；用反向提示词掩盖上游持有者或左右手事实冲突。
...
### R15-M Negative Placement Scatter Rule

输入：一个三阶段Clip——阶段1听到门外脚步、需要控制声音（`无bgm，只生成环境音和动作音`）；阶段2手部接触道具、需要锁定左右手；阶段3画面内不出现额外字幕条。

PASS：声音控制写在阶段1的`音效`附近；左右手约束写在阶段2的`道具状态` / `人物动作与情绪`并优先写成持续正向状态；字幕控制在字幕出现位置写入（本项目字幕按后期叠加时写明该边界，不新增字幕字段）。末尾`全局限制与反向提示词：`只保留固定禁BGM首句、阶段总数兜底句与真正跨阶段、正向锁不死的残余类别。位置依据是Seedance 2.5官方文档的负面提示词散布规则（取证日期2026-09-22，能力依据与取证缺口见`adapters/seedance-2.5.md`：该截图为整理稿，未附文档标题、版本号与发布日期，属待补项）。

FAIL：把这三项内容约束全部抽离所属阶段、堆进末尾反向段；把阶段总数兜底句或固定禁BGM句改写到任一阶段正文；为了“分散”而在每个阶段重复同一条通用限制；新增字幕字段承载字幕控制。

### R15-N Stage-Internal Writing Format

输入：一个15秒四阶段的情感文戏Clip，阶段内需要同时固定机位参数、台词时点与八维表演。

PASS：`画面与镜头`以`镜头01｜50mm T4，双人中景固定机位，两人与房间结构同时可读`这类写法开头，焦段与光圈紧接它们换来的可见结果；镜头编号只在`画面与镜头`内随阶段写一次，阶段头保持纯`[0—2.5秒]`。`人物动作与情绪`按`1 时间｜… 2 目的｜… 3 情绪变化｜… 4 触发｜… 5 面部｜… 6 视线身体手｜… 7 声音呼吸｜… 8 结束状态｜…`逐项写，维度名用`｜`；`结束状态`与`阶段结尾状态`各写各的内容，同一句不重复。负荷轻的阶段合并为3—5项。PEX / AU编号译成可见动作（AU23→眉心收紧、AU43→闭眼约0.3秒），不进入正文。

FAIL：把维度写成`时间：`、`触发：`等带冒号的行——分镜或阶段块内每一行`名称：`都会被当作字段名与固定字段逐项比对，多出即报字段缺失/超量；把镜头编号或焦段塞进阶段头导致`[N—M秒]`形态被破坏；写`T4`而不写它换来的景深或明暗结果；把PEX / AU编号、PL等级或Performance Arc Map术语写进正文；用编号维度为形式完整虚构当前Clip没有的表演节拍。
