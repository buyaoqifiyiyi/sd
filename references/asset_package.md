...
包在主Pipeline中的位置是固定的：它是**STATE-08最终Prompt交付轮的正式收尾交付物**，随该轮自动执行，不需要用户另行下令——它是流程的正式组成部分，不是可选附件。
...
包是主Pipeline的正式收尾交付物：固定在STATE-08最终Prompt交付轮自动执行，不需要用户下令
未进包的要逐项报告原因，不静默丢弃
