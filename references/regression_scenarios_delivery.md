...
输入：一个Seedance 2.0 Clip，末尾`反向提示词：`声明分镜总数（“本Clip严格只有N个分镜，禁止增加额外镜头、额外地段或额外场景”），逐分镜区块数量与之不一致；以及一个模型可能扩写单镜的情形。

PASS：末尾反向提示词声明的分镜总数与逐分镜实际数量完全一致，且该声明是该数量限制的权威落点；存在扩写风险时兜底句保留在同一字段。Seedance 2.5由`时间线：`首行声明阶段总数、MiniMax H3由`画面过程说明`首行声明段数，各自与末尾反向约束字段的同义兜底句一致。

FAIL：缺少总数声明、虚报数量，或新增独立字段（含恢复`画幅：`）承载数量声明而改变STATE-08固定Schema。
