...
PASS：STATE-06先写出三段式情绪变化`起始状态 → 变化触发与过程 → 结束落点 / 观众残留感受`，并指出触发来自哪个可见刺激或表演Beat；再指定唯一主要承担变量（例如随确认发生的距离变化），其余变量保持或明确让位并说明理由；主运镜写出`Trigger → Path（含Mid-path Change）→ Stop → End Composition`与运镜理由，或给出有理由的Static / Locked-Off所保护的表演与信息。抽象词已改写为可执行的连续视觉动作与构图变化，STATE-08仍只使用既有字段，不输出内部标签。**STATE-08编译时，每段`画面与镜头`还要写出触发者（是谁的哪个可见表演/情绪事件）与可核对的起止（`中景 → 近景`、`两人距离 → 手与脸`），并保证整片至少一次由关键情绪节点驱动**；纯功能性触发（走过门框、进出门、雨声、脚步）不替代这一次情绪响应。

FAIL：只标注“压抑 / 震撼 / 氛围感”并据此推进；九个画面关系变量同时改变而没有主承担者与让位理由；主运镜只写“缓缓推进 / 轻微横移 / 环绕”而不给Trigger、Path、Stop、落点与运镜理由；或为凑情绪叠加无理由的快环绕、频繁推拉、炫技转场与持续运动。**STATE-08把所有阶段的运镜都写成空间或功能触发，或只写“固定观察 / 跟随 / 停住”而不写触发者——成片看起来就是“没有情绪运镜”**，此时按`knowledge/prompt_compilation/state08_projection.md`Director Pass第5项返回Director / STATE-07补，不在Prompt里自己发明。
