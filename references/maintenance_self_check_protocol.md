...
本文件是Skill维护QA的**判据真源**：它拥有`references/maintenance_self_check.md`中18个检查维度每一项的完整判据、边界、反例与处置要求，以及两个固定Guard的完整协议。
...
8. **Reference Integrity Check**：验证所有显式文件、模块、Template、Knowledge与脚本路径真实存在且名称一致；新增资源已被合法路由发现；删除或改名后没有悬空引用。**射程必须写清**：确定性引用检查只覆盖**带skill目录前缀**的引用（`rules/…`、`knowledge/…`等）；**裸文件名**（`foo.md`）不在射程内——发布文件永远带目录前缀书写，裸名在这套文档里指项目内运行期产物（如`shots/director_decision_notes.md`、`artifact_registry.md`），它们只在项目里存在，无法从skill根核验。因此**删除或改名任何文件后，必须自己全库grep一次文件名**，不得因为"validator通过"就认为没有悬空引用；`r79`删除覆盖表时就有一条反引号裸名引用靠人工grep才发现。**同时反向检查孤儿内容**：新增的字段、规则或知识必须能被某个消费者读到——被某个Workflow列为Resource、被某个Template使用、或被某个上游事实引用；定义了却无人引用的内容等于已经丢失，必须在本轮补上消费者或移除它。**孤儿检查同样适用于文件，且每次必做**：一个已发布文件如果从入口（`SKILL.md` / `config.md`）出发既不可达、又未声明自己属于`Skill维护层`，就是不可读内容，必须路由它、声明它或删除它；`USER_GUIDE.md`是唯一已登记的例外（非运行时文档）。豁免必须显式且可见——`USER_GUIDE.md`这类非运行时文档与自己声明`Skill维护层`的文件都要在报告里列出来，否则“有意跳过”与“漏掉”无法区分。这条反向判定由`scripts/validate_sd_film.py`的`check_reachability`确定性执行——它只证明"可达或已声明"，不证明内容正确。运行`scripts/validate_sd_film.py --skill-root <skill-root>`执行可确定的结构与引用检查。生产交付物校验由独立owner`scripts/validate_prompt_package.py`承担：它在STATE-08交付前对已编译的`# CLIP-X｜…` Package做确定性结构断言（全局字段存在与顺序、分镜或阶段编号连续且数量与声明一致、`REF-TAIL`用途声明、终段位置与固定无BGM句、未授权`音色特征：`，以及三条内容形态：Canonical参考条目形态、`主风格`不得保留通用负向清单且须带四锁维度名、多阶段一镜到底不得通篇小幅缓动）。其姊妹owner`scripts/validate_delivery_artifacts.py`守护STATE-05/06/07的用户可见交付物：`--kind scene-breakdown`验证SCENE编号连续、逐场Scene Directing Brief六个子项与七个Template区块；`--kind shot-design`验证默认5列表或完整十八列表、SHOT编号连续、每格非空；`--kind clip-plan`验证默认6列表、CLIP编号连续、`包含镜号`引用正式SHOT。三者职责互不替代：`validate_sd_film.py`守护Skill自身，这两个守护本次交付；它们都不进入本Checklist的维护QA执行owner。
...
15. **Context Budget Check**：`references/context_budget.md`是可达性判据、文件类别与Size Index的唯一owner，本项只引用它，不复制数值。**越线不是违规**：复核线与Ceiling的区别就是“提示”与“阻断”的区别。按`Maintenance System Map`定义的三层节奏执行本项：
    - **事前（Prevent）**：新增内容先归位到既有owner，不因“方便”而新建文件；新建Markdown不得超过复核线的60%（30 KB）；本次变更若使文件越过复核线，必须在同一次变更内执行一次**瘦身优化**（按`references/context_budget.md`的`### 瘦身优先序`：先退休与唯一 owner 重合的内容，再合并，再压缩；只搬迁到别的文件不算瘦身），瘦身后仍越线才拆分或给出读取入口。
...
16. **Claim / Evidence Credibility Check**：核对**用来判断这次改动是否成立的那份证据本身**是否可信。本项不评价改动内容，只评价支持它的证据。四类必查：
    ...
    - **判据必须锚定结构位置，不能只匹配字符串**：任何"某处必须存在 X"的检查，先问 X 在文件里**是不是只有一种结构角色**。同一串字符常常同时是正文引用、表格里的名字、`# Read Scope` 里的反引号提示与真正的标题；只做子串匹配时，**删掉真标题仍会命中正文引用而报 PASS**，**正文里多写一次又会被当成重复登记而报 FAIL**。同一模式在 r95—r98 之间实测出现四次：frontmatter 的内层引号让宿主静默丢弃整个Skill，而"`name: sd-film` 与六个别名都还在"的子串检查全绿；`templates/04` 的小节标题在正文被引用三次，删掉标题后仍通过；索引正文一句"展开见 `<atom>`"被判成第二次登记；改为只读花名册节后，又因 `# Read Scope` 表里反引号写出的节名而把全部登记读成缺失。处置：判据锚定到**行首标题、表格行或指定小节**，并为每条这样的判据配一个"目标只出现在错误位置"的反例测试。
...
18. **FAST Invariant And Delivery Receipt Check**：核对**自动模式是否仍然只是"自动确认"**，以及交付收据是否仍在各自的 owner 里。触发这一项的是实测违规：一个 FAST 项目把 STATE-05/06/07 压成一句"已完成场景、镜头与执行规划"就直接给 Prompt，三份完整工件都没出现——而`rules/automation_mode.md`当时已经写着"不能以摘要代替"、"不得预先生成最终Prompt"。**规则不缺，缺的是把不变量写成一句硬话 + 让缺件在交付时可见的收据**，而且这两样必须同时在五个文件里活着，所以它进入维护自检。
    - **五处必须同时在位**：`rules/automation_mode.md`（`## Purpose And Owner`的"FAST只自动确认，不减少流程与产物"、连续链的"也不减少阶段与交付物"、聚合包的"合并的只是展示切片"、`### Delivery Receipt｜交付收据`的三种状态`本轮完整输出` / `已在Accepted Artifact` / `待交付`，以及`Prompt纪律自检`条目（逐条`过 / 不过 / 不适用` + 证据，只提高可见性、不升为 Hard Gate））、`rules/05_output_rules.md`（交付清单不因FAST改变 + 指向收据）、`references/project_state_contract.md`（`COMPLETE`必须有实际产出证据，"仅有一句『已完成×××』声明不构成证据"）、`references/module_contracts_auxiliary.md`（"自动接受的是确认，不是工件"）、`USER_GUIDE.md`（用户侧同一口径 + 遇到一句话顶替完整工件时怎么说）。
    - **判定方法（人工，任何环境都必须做）**：读到任一处的自动模式说明时，问两句——"它有没有说自动模式可以减少阶段 / 检查 / 交付物？"（有即发现项）与"聚合交付有没有要求列出阶段 → 工件 → 状态？"（没有即发现项）。
    - **边界**：本项只保护不变量与收据的**在位**，不判断某次交付是否照做（那是运行时行为，回归由 R32-D / R32-E 与 `### Delivery Receipt｜交付收据`的判定纪律覆盖）；收据是交付核对，不得被写成新的 Project State 字段或替代 Template。
    - **射程必须说清（Claim / Evidence）**：确定性部分由`scripts/validate_sd_film.py`的`check_fast_invariant_and_receipt`执行——逐条验证上述句子与状态标签仍在各自文件里（任一丢失即 FAIL）。它**不证明交付收据真的被使用过**，也不判断"自动接受"是否留下了Artifact / Version History证据。删除某处句子使标记缺失会被直接判FAIL，不得作为通过方式。
...
- frontmatter必须真的能被YAML解析，而不是"文件里含有这些字"。实测失效：把一个`description`的中文引号写成直引号后，它嵌在同样用双引号包裹的标量里，YAML在第一个内层引号处提前结束标量，宿主解析失败后**静默**把整个Skill从列表里丢弃——`name`、六个启动别名、重复入口三项子串级检查此时全部保持绿色，所以"改完文案仍能被发现"必须由解析本身证明，不能由"别名还在"证明。内层引号只能写成转义`\"`或全角引号。本项确定性owner为`scripts/validate_sd_film.py`的`check_skill_frontmatter`。
