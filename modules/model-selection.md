...
位置：STATE-00确认项目级视频模型偏好，STATE-06 Detailed Shot Design确认后、STATE-07 Clip Planning前复核该偏好是否适用于当前Clip；它不创建新STATE。

1. 在STATE-00的`Project Model Selection Proposal`中，与项目图像模型默认项一起展示视频模型偏好：Seedance 2.0 / Seedance 2.5 / MiniMax H3、对应Adapter、已知时长和参考能力边界。用户已在项目请求中指定时只展示该候选；否则不默认选择。确认后写`Project Video Model Preference`，不是Clip执行Profile。
