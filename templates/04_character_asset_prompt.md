...
Core与Support共用上述双确认Gate。Core使用一张独立的正式角色资产设定图：同一画布内固定为五个区域——上排三个等宽等高的全身区（正面、严格侧面、背面，共用同一人物尺度与水平基准线），下排两个更大的头肩特写区（中性表情、微笑表情）。媒介为`2d_anime`时不使用五区版式，改按`#### 2D Character Asset Sheet Prompt｜设定集与画风锚`的三区块结构（角色设定区 / 表情区 / 画风与色指定区）。不得将其拆为独立分区图、独立面部特写或独立表情图Candidate Reference。Support按Board制作，不得逐个生成完整正式角色资产图或独立面部特写。Board图片确认前，Board及任何Item的`Confirmed Status`都必须为`No`。
...
**媒介适用条件**：该五区结构服务于画面由光生成的媒介（`live_action` / `3d_animation`），其"正面区交出面部与头发"的前提是下游图像模型的身份采样。媒介为`2d_anime`时**不适用**：2D角色资产由设定集与画风锚承担，正面区必须完整含头部与面部，不得套用面部单源逻辑；该档结构见下方`#### 2D Character Asset Sheet Prompt｜设定集与画风锚`。媒介判定与三档分化规则见`knowledge/medium_profiles.md`。
...
仅当`媒介形式`为`2d_anime`时输出，**取代**上方的五区结构。绘制媒介的身份锚是**形状与色指定**，画风锚是**线稿与上色**；因此本档不套用五区版式。判据与等效词汇见`knowledge/anime_language/04_style_and_consistency.md`与`knowledge/medium_profiles.md`的Aesthetic Layer。

同一画布内固定为三个区块：

**角色设定区（上排，三个等宽等高全身区）**：正面、严格侧面、背面。三区共用同一人物尺度与同一水平基准线（肩线、腰线、髋线、膝线同高），**三区均完整包含头部、面部与发型**；任何一区都不得挖空头部、省略正面五官或把头发移出该区。

**表情区（中排，3—5个等大头肩区）**：固定同一视角、同一构图、同一线条处理与同一着色法，**仅表情不同**；至少含中性表情与项目实际需要的情绪。表情变化不得使脸型、年龄感、颧骨或下颌宽度漂移。

**画风与色指定区（下排）**：线条层级与线宽、上色法（平涂 / 渐层 / 水彩边界）、阴影层数与网点或笔触处理，以及主色 / 暗部色 / 强调色的色块关系与明暗层次。色指定用**可命名色块**表达；用户未提供色值时不得写出Hex、RGB、LUT或色域参数（沿用`knowledge/color/foundations.md`的既有纪律）。

画风锚必须写成**可执行项**，例如"统一线宽的一级线稿 + 二层cel阴影 + 暗部暖灰网点（中等密度）"；不得使用"日式赛璐璐""高级感""低饱和"这类无法复核的形容。

Prompt必须写全主体、三区块构图与区块关系、视角、姿态、表情基线、着色法、背景、一致性限制、必要负面限制与生成参数，不使用"同上/参考前述"。禁止输出独立的Three-View Prompt、Face Close-Up Prompt、Expression Sheet Prompt或Color Sheet Prompt，禁止基础正式资产分两张或以上生成。

**后续一致性依据**：本档的Canonical一致性依据是**设定集（形状与色指定）+ 画风锚（线宽、上色法、网点）**。下游阶段、重生请求或QA**不得**把上排正面区含头部与面部判为失败——这与五区结构的判据相反；也**不得**把实拍五区结构套回本档。画幅沿用人物类默认`9:16`竖版；竖版不得压缩三区块、不得裁切头顶与脚。
