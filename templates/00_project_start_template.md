...
说明：此处是项目级默认/偏好。STATE-03直接继承图像默认项；STATE-06后仍按每个Clip的时长、首尾帧、编辑模式和真实参考输入能力复核视频偏好。选择不等于资产图片确认，也不提交外部服务。图像交付形态只决定这一轮交付图片还是交付Prompt：`AUTO`按当前执行环境的真实出图能力路由，未设置时即按`AUTO`，不得用历史推断。
