...
| 台词 | `台词 / 旁白 / 口播` |
| 环境声、音效 | `音效 / BGM`内分别使用`环境声：`、`同步音效：`，并写声音尾部与后期BGM规划 |
| 光线 / 色彩 | `光线 / 色彩` |
...
先分别写`人物情绪：`与`动作重点：`，再按可观察顺序填写：`起始状态 → 刺激/注意 → 反应 → 决定/动作 → 稳定结束状态`。有人物时同时记录视线目标、呼吸、手部/重心、一个主要面部变化、一个支持身体变化、表演强度和动作结果；对白、哭笑、吞咽与呼吸必须排出先后。
...
BGM默认只供后期剪辑，不进入STATE-08正向音效。只有用户明确要求Seedance生成指定Clip的BGM时，才由STATE-08按既有例外规则处理。
