...
- Organization Mode：单Shot / 多Shot连续生成 / 多Shot有动机剪辑；后者记录唯一Cut Contract并在容量不足时拆分Clip
...
- 包含 Clip 表列出的1个或多个`分镜X`；单Shot按独立镜头执行，多Shot连续生成写成同一次长镜头的连续阶段；多Shot有动机剪辑仅在每个现有逐镜字段明确切点/媒介、切前结束、切后稳定重建和连续性锚点时执行
- `参考资产：`只列预算审计通过的当前Clip实际真实资产，以及A/B必需但尚待用户补充的`REF-TAIL`受控声明；默认Projected Final Count≤9，2.5扩展时遵守已验证有效图片上限和30图 / 10视频 / 10音频 / 合计50审计；待补充尾帧不计入已提交图片数，核心角色各自保留独立角色资产图/角色锁定图，非角色整合仅在超限风险触发后执行
- 保持来源分镜编号、顺序与逐镜字段完整
- 使用`# CLIP-X｜标题 Seedance视频提示词`作为区块标题，在`时长：`写明用户选择的模型适用平台生成时长（2.0为4—15秒；2.5为4—30秒，16—30秒须严格预检PASS）；不得创建独立CLIP标题字段
