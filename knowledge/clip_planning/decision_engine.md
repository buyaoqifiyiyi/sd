...
3. 若加入后超过当前锁定模型的时长窗口上限（Seedance 2.0与MiniMax H3为15秒；Seedance 2.5为30秒），立即结束当前Clip；不得为追求多镜Clip而压缩动作或删减覆盖。Seedance 2.5的16—30秒区间不是硬门槛，但必须先通过严格预检（`templates/20_clip_plan.md`的`Long-duration Preflight`）；预检FAIL时按第7步拆分为4—15秒Clip，而不是在合并阶段就提前压到15秒以内。
...
7. 候选Clip只要总时长符合已锁定模型窗口且边界完整即可确认：2.0为4—15秒；2.5为4—30秒，16—30秒自动严格预检。单Shot少于4秒时尝试与相邻兼容Shot组合；仍不能成立则返回STATE-06修正，不得靠新增剧情动作凑时长。2.5的16—30秒任一严格预检失败时返回STATE-07按自然边界拆分为4—15秒Clip，不改写已确认Shot；超过30秒或2.0超过15秒才返回STATE-06拆分。
...
| Duration | 2.0为单镜或多镜合计4—15秒；2.5为4—30秒，16—30秒须严格预检PASS |
