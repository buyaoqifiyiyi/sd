...
- **Clip**：一次视频模型生成调用的执行单位，可包含一个Shot，或多个按原顺序排列、可连续执行且合计符合已锁定模型窗口的相邻Shot：2.0为4—15秒；2.5为4—30秒，16—30秒须严格预检PASS。
...
硬切、跨时空、跨资产版本或重建摄影机默认应拆Clip；只有通过上述多Shot有动机剪辑合同且模型容量支持时才可留在同一Clip。任何一个分镜只要自身符合已锁定模型窗口都可独立成为单Shot Clip；2.5的16—30秒仍须严格预检PASS。
...
每个候选Clip还必须按`knowledge/reference_budget.md`建立图片参考预算。默认保留原始独立资产，只有Projected Final Count出现接近或超过9张上限的风险时才整合同类非角色信息；核心角色独立角色资产图/角色锁定图不得合并或由动作图替代。预算失败不通过Clip确认。
