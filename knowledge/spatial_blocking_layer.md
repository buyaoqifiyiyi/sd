...
Spatial Blocking Layer 用“Text Spatial Rules + Top-down Blocking Map（按复杂度启用）”在正式 Detailed Shot Design 之前锁定角色站位、摄影机位置、关系轴、移动路径、关键道具和跨 Clip 边界，减少左右漂移、反轴、无动作换边、瞬移与首尾帧衔接错误。
...
- **Output**：每个 Scene 一份 `Spatial Blocking Result`，至少包含 Decision、Structured Blocking Map、Text Spatial Rules、Clip Boundary Spatial Ledger，以及适用时的 Top-down Blocking Map Prompt / 图像。该结构是生产工件，不是新的 Shot、Clip 或 Seedance 输出 Schema。
...
俯视图和文字地图必须使用同一方向约定，例如：`图上方 = 场景北；图下方 = 场景南；摄影机朝向用箭头；人物面向与移动路径分别标记`。画面左 / 右是具体 Camera View 下的屏幕结果，不能与场景东 / 西混写。
...
- 摄影机不得越过哪条 180 度轴线；有意越轴时必须记录已建立轴线、中性机位 / 连续可见越轴路径、固定地标和稳定新轴线侧；
