...
- 镜头目的
- 情绪功能与情绪变化（起始状态 → 变化触发与过程 → 结束落点 / 残留感受）
- 镜头语言母题节点（出现 / 变化 / 反转；未到节点写Not Applicable）
...
### Movement Novelty Budget

复杂度是稀缺资源，不是镜头质量的替代品。每个Clip最多占用**1次**运动新奇度破例（Orbit / 360、穿行、大范围升降、连续越轴、多段一镜到底中的任一项），且必须绑定该Clip已确认的视觉高潮镜头；其余镜头用距离、侧位、遮挡、焦点与`index.md`的Movement Phase制造差异，不再升级运动复杂度。

本预算与STATE-08的`Generation Budget`分工不同：后者控制模型负荷与参考输入，本预算控制观众的新鲜感与风格排他性。两者同时吃紧时先降运动复杂度，保留身份、动作可读性与信息时机。破例未绑定视觉高潮、或同一Clip出现第二次破例时，该Camera Language Decision不得确认。
