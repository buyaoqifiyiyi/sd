...
### Layered Depth And Readability

当当前Shot需要同时让观众读到人物与空间时，先把画面落实为可验证的层次：近景的遮挡/引导物、中景的主要人物或行动、远景的环境锚点；不需要三层时至少明确主体与一个空间参照。每层必须说明其叙事或注意力功能，而不是为了“有纵深”凭空添加门框、雾、粒子或装饰。

焦段、摄影机距离和景深只服务这一层次关系：前景可轻微柔化，但不应遮断必须同时成立的人物、关系或环境信息；需要身份、表演与场景共同可读时优先足够景深。真实推进、侧移或小角度弧线必须让前后景产生连续视差，并在明确Stop处落入稳定构图；不得写成无空间依据的数字缩放或漂浮运镜。

### Adjacent Observation Contrast And Deliberate Repetition

相邻SHOT不因“避免单调”而强制换角度、景别或运镜；但当Shot Purpose、Audience Attention、POV / Audience Position、关系状态、信息阶段或节奏功能发生变化时，必须检查观众是否能从至少一个可见维度读出变化：景别 / 摄影机距离、观察侧位或高度、构图与前中后景、焦点与遮挡、Static / Hold或主运动路径。

若相邻镜头承担不同功能却仍保持同一平视中景、同一正面或后方观察、同一路径跟随和同一节奏，必须为这种重复写明连续长镜、压迫累积、关系冻结或信息压住的具体理由，并写出何时解除或落入稳定终点；不能说明时，返回Shot Design重做最小受影响镜头。不得为了制造差异而无动机越轴、环绕、升降、甩镜或随机切近景。

当一个Clip覆盖三个及以上明确戏剧阶段时，STATE-07必须读取这项检查并在既有Clip Movement Plan中保留阶段间的观察层次；若只有一个统一摄影机逻辑能够合法承载全部阶段，必须记录其连续长镜理由、受保护的注意力对象与稳定降级。否则按既有Clip边界拆分或采用有动机剪辑，不把“同一台摄影机自然跟随”当作默认解。
