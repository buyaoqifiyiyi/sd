...
**`2d_anime` 禁止项**：不得为该档镜头指定焦段毫米数、光比比值、轨道/摇臂设备、稳定方式或真实景深。这些在绘制媒介中没有对应物，写入只会污染Prompt并误导生成，必须改用等效表达；**等效词汇的owner是`knowledge/anime_language/index.md`**（版面与空间、运动的版面等效、帧感与冲击、画风锚与一致性），本表只列出分化维度与示例词，不重复那套词汇。
...
**资产形态也随媒介分化**：`live_action` / `3d_animation` 的角色正式资产为五区资产图；`2d_anime` **不套用**该结构，其角色资产是设定集与画风锚。结构与判据细节归`templates/04_character_asset_prompt.md`的`#### 2D Character Asset Sheet Prompt｜设定集与画风锚`（三区块：角色设定区 / 表情区 / 画风与色指定区），本文件只声明适用档位。
