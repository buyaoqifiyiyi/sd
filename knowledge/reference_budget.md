...
- 上一Clip尾帧只有在Preflight分类为`视觉连续`、Handoff为Direct或Reference-Only，且实际尾帧图存在、可访问并已确认可用于当前生成时，才属于必须占位的连续性图片；统一以`REF-TAIL-XX｜CLIP-XX尾帧参考`登记。没有实际尾帧图时不得以名称占位、假路径或文字End State冒充图片，只做文字承接。`剧情连续`或`主动切场 / 切世界`不得机械加入上一尾帧或为其预留图片位。
- Storyboard、多格分镜板、线稿、拼图、接触表、Blocking Map与设计表截图继续服从既有禁用规则，不因预算紧张而获得引用资格。

## Conditional Trigger Thresholds

先按World-State删除不适用资产，再统计当前真实候选数；仅加入已经由`视觉连续`分类确定必须占位的首帧、上一 Clip 尾帧或其他合法连续性图片需求，得到`Projected Final Count`：
...
5. 仅当主分类为`视觉连续`时，核验Direct / Reference-Only所需的上一Clip尾帧、当前首帧或其他合法连续性图片。实际存在、可访问且已确认的上一尾帧按`REF-TAIL-XX｜CLIP-XX尾帧参考`加入；未实际存在的只可记录为文字承接或待取得需求，不得进入最终图片清单或已提交计数。剧情连续或主动切场不得加入旧尾帧。统计`Projected Final Count`。
...
| F. Direct连续但无实际尾帧图 | 不创建`REF-TAIL`占位、不计图片位；最终`参考资产`不列该尾帧，改由`首帧参考`文字承接上一Clip End State |
