...
- 上一Clip尾帧只有在Preflight分类为`视觉连续`且Handoff为Direct或Reference-Only时才属于必须占位的连续性图片。`剧情连续`或`主动切场 / 切世界`不得机械加入上一尾帧或为其预留图片位。
...
5. 仅当主分类为`视觉连续`时，加入Direct / Reference-Only已确定必须使用的上一 Clip 尾帧、当前首帧或其他合法连续性图片需求；剧情连续或主动切场不得加入旧尾帧。统计`Projected Final Count`。
