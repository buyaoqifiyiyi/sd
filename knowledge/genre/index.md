...
## Craft And Formula｜工艺与公式

本模块的禁止对象是**公式**，不是**工艺**。两者的分界固定为：

| | 形态 | 例子 | 处置 |
|---|---|---|---|
| **Craft｜工艺** | 一种手段，连同它的成立条件与反用场景——它回答"存在什么手段、在什么条件下成立" | "当两人对话僵住时，可以引入第三件事让关系通过它变化" | 允许存在；写作纪律与类型倾向相同：条件不满足即不得使用 |
| **Formula｜公式** | 一条时序或密度规定，不带条件——它回答"第几分钟必须发生什么" | "第8分钟必须反转""类型片必须三幕""每场必须有钩子" | 禁止 |

判据只有一条：**这条内容能不能被写成"在什么条件下、做了什么、观察到什么结果"。** 写得出是Craft，写不出、只能写成时间点或数量的是Formula。

因此本模块的禁令**不延伸**到Writer层的工艺手册（`knowledge/writer/screenplay_development.md`的`## Craft Manual｜工艺手册`），也**不构成**"故事结构领域不可被讨论"的声明：类型**呈现**（镜头、表演、声音、节奏倾向）归本模块；人物、结构、场景与对白的**工艺**归Writer Owner。两者正交，互不覆盖。

## Anti-Formula Discipline｜反公式边界

本模块**明令禁止**以下写法与用法。它们也是回归`R24-J Genre — No Universal Conflict Formula`的直接对象：

- **禁止固定节拍模型**：不得出现"第N分钟必须反转""每场必须有钩子""类型片必须三幕"这类与项目无关的时长或密度要求。本禁令的对象是Formula形态；带成立条件的Craft条目按上一节的分界处理，不在此列。
...
- 类型承诺与已锁定剧本冲突 → Writer Owner（`knowledge/writer/screenplay_development.md`）
