# Seedance 2.5 Model Profile

> Legacy compatibility entry. `adapters/seedance-2.5.md` is the active 2.5 capability owner and is read only after Natural Clip Planning. If this file differs, the adapter wins.

## Scope And Evidence Boundary

本Profile只在Model Execution Lock选择`Seedance 2.5`后由STATE-07/08消费。依据用户提供的火山方舟官方《Doubao Seedance 2.5 提示词指南》《Doubao Seedance 2.5 教程》与Dreamina《Seedance 2.5 User Guide》，它记录可核验的模型能力上限：单次生成最长30秒、最多50个参考素材（最多30张图/10段视频/10段音频）、时间戳文本控制、纯音频驱动、多轮Video Extension、Clay Render/白模，以及更强多镜头叙事。它不声明未证实的API字段、上传格式、网关参数或所有第三方入口均可用；但用户选择的4—30秒时长在规划阶段不因网关状态被预先压缩，平台拒绝仅作为实际生成后的反馈处理。

## Execution Modes And Automatic Duration Route

- `Standard Clip`：4—15秒的稳定短Clip工作法，沿用既有Preflight、Reference Budget、A/B/C REF-TAIL和End-State合同。
- `Long-form Clip`：是Seedance 2.5的内部时长路由，不是用户额外选择的Execution Mode。用户目标时长为16—30秒时自动触发；只有镜头链、空间关系、表演连续性、动作/物理密度均通过更严格预检才启用。任一风险失败即在STATE-07拆回多个4—15秒Clip。
- `Video Extension`：以实际上一段已生成成片作为受控`REF-VIDEO`输入延展。它叠加于Canonical资产、首帧/尾帧、资产锁和End-State之上，绝不替代它们。
- `Targeted Edit`：仅用户明确要求修改既有视频时启用。可在既有分镜正文写受控时间段、修改什么和必须保持什么；不新增时间轴字段或模型字段。

## Reference And Authority

最多30图、10视频、10音频、合计50个参考是Seedance 2.5默认的容量边界。每个实际上传素材必须在内部Reference Audit中登记来源、类型、唯一Primary Role、授权维度和当前Clip收益；没有唯一职责的素材不得提交。Reference Selection / Routing仍采用最小充分原则，并受实际网关限制；“最小充分”只减少无效输入，不再把2.5限制为9图工作预算。音频参考仍须用户明确要求当前视频使用声音、音色、动作、口型或节奏控制；纯音频驱动不得自动启动，也不改变Voice opt-in或视频Prompt永久禁止背景音乐、配乐、BGM。

## Timestamp And Dreamina Surface Boundary

Seedance 2.5的时间戳文本控制可用于普通多Beat、蒙太奇或复杂连续镜头，不局限于Targeted Edit。仅在能提高动作先后、场景转换或节奏节点可读性时，按Clip时长在既有分镜正文写递进的`0—3秒 / 第3秒`语义；不新建`时间轴`字段、逐帧参数或独立分段模板。简单镜头继续省略。

Dreamina网页端指南还声明30—180秒一键长视频、画布标注编辑、绿幕、双视频无缝转场和多格分镜。这些是**入口受限能力**：只有用户明确指定Dreamina Web、实际入口可访问并提供相应素材时，才输出适合该入口的提交方案；常规Seedance 2.5 / 方舟路径保持4—30秒，不得借用这些网页能力。

经验证的无性别`REF-SKETCH`在2.5中可作为Clay Render/白模空间调度参考，但只拥有Position、Facing、Distance、Topology、Camera、Pose、Gaze和Action Path Authority。它不得覆盖Character外观/年龄/服装、Environment材质、灯光、色彩或最终画风；Character / Environment / Prop Canonical Authority与双确认继续优先。

多镜头叙事能力不放宽复杂多人交互、物理或动作风险。出现负荷过高、连续性不确定、状态重置或空间轴线风险时，执行既有拆分/降级并保留已确认上游事实。

30秒是单次能力上限，不是默认时长，也不自动等于一镜到底；一镜到底仍须由已确认的镜头设计和严格预检证明摄影机路径、空间关系、表演连续性及动作/物理密度可执行。用户可在4—30秒内选择实际时长；未知网关状态不再自动回退到15秒。

## Native Compilation Semantics

2.5与2.0的差异由`knowledge/prompt_compilation/seedance_25_compilation.md`消费：官方资料支持按参考素材用途明确映射主体、动作、Clay Render、风格、音频、分镜格、关键帧及编辑/延展任务。映射只在内部保存实际素材的来源、顺序（如入口需要）、用途、Authority和可用状态；能力存在不等于当前网关开放该任务或角色。最终输出仍只经共享Projection写入固定Template，不暴露API字段、内部参考角色或任务标签。
