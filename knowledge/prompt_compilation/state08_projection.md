...
**写法固定**：第3项在字段内保留四个维度名（`反差与光比结构`、`色彩对抗关系`、`构图主张`、`视觉母题与变化轨迹`），每名后写本项目的选择与它排除了什么可见结果——这样交付前可以逐项核对，也便于`scripts/validate_prompt_package.py`在字段完全没有维度名时给出提示（该提示不阻断交付；部分缺失仍由Output QA人工判定）。裸`不X`式通用边界（如"不破败、不阴森"）能改写成正向状态的一律改写，只有四锁里必需的"排除了什么"保留否定式。
...
当某个节拍的失败风险不是"违反某条约束"，而是被生成成**另一种相近但错误的动作或表演读法**（例如把"持刀静立、只做呼吸起伏"生成为大幅挥砍，把"定格凝视"生成为夸张瞪眼）时，Positive Specification必须落到该节拍**可枚举的小载体**，而不是只把否定词反写成形容词：写清姿态是否稳定、有无真实呼吸起伏、发丝与衣物的自然摆动、视线移动速度等具体可见元素，并写入该阶段的`画面与镜头`或`人物动作与情绪`。判断方法是问一句"这个节拍被生成错时，最可能错成什么"，把该错误读法对应的正确小载体逐项写进正向字段；该风险的负向部分按Negative Placement**归位**：属该节拍内容的部分留在其所属字段（以最短否定句或持续正向状态），只有真正跨阶段、无法下放到任何单一节拍的部分才收束到末尾唯一`反向提示词：`，并按其失败**类别**合并，不逐条罗列实例。
...
2. 能定义目标状态的改写为正向执行要求；**负向控制的默认位置是它所约束的内容所属的阶段**——与具体动作、空间关系、道具状态、表演、光线、声音或字幕相关的约束写在该阶段对应字段内（字幕控制在字幕出现位置写入，音频控制在需要控制音频的段落附近写入），作为情节描述的自然语言一部分；只有真正跨阶段、不归属任何单一阶段的通用禁止项、错误规避项和负向清单才移入末尾唯一`反向提示词：`。
3. 阶段内保留的内容约束必须最小必要且指代清楚；优先写成持续正向状态，例如`左手持续握住伞柄，整个动作链保持左手持有`。若仍需最短否定说明，只约束该局部，不扩展通用清单，也不把它抽离到末尾段。
4. 合并同类高风险项，删除与正文重复、低价值、抽象化、同义重复或与当前Clip无关的负向词。除固定平台边界与阶段 / 分镜总数兜底句外，只保留真正跨阶段、正向状态仍难锁死的少量当前高风险错误；反向段不得比主体、动作、空间、摄影机、时间顺序和Handoff更抢控制位置。
5. 每个Clip只允许一个`反向提示词：`，且它必须是最后字段和最后段落；其后不得输出任何正文。

第2—3项的默认位置来自Seedance 2.5官方文档的负面提示词散布规则：负面提示词写在正文的具体情节描述内，逻辑上作为自然语言的一部分融入情节，不单独成段；字幕控制写在字幕出现的位置，音频控制写在需要控制音频的段落附近。因此内容相关约束的首选位置永远是它的所属阶段，末尾字段不再是默认收容所；能力依据、取证状态与待补项见`adapters/seedance-2.5.md`。
...
| Cross-shot Risk | 反向提示词；各内容所属阶段字段 | 内容相关的负向控制写在所属阶段；末尾唯一`反向提示词`保留永久固定禁BGM首句、阶段 / 分镜总数兜底句，以及真正跨阶段且正向锁不死的残余风险类别；不存在音乐例外 |
...
矩阵中凡把`反向提示词`列为目标字段的行，只指该来源里真正跨阶段、无法下放到任何单一阶段的那部分语义；与该来源内容相关的负向约束仍按本文件`### Positive Specification And Negative Prompt Placement`写在它所约束的阶段字段内（例如空间关系的边界写在`空间关系`，道具持有者写在`道具状态`），不得整体搬进末尾段。
...
- 能以正向状态锁定的约束是否已经执行`Negative Constraint → Positive Target State`；负向控制是否写在它所约束的内容所属阶段，末尾反向提示词是否只保留固定平台边界、阶段 / 分镜总数兜底句与真正跨阶段且正向锁不死的残余项，而不是所有可能错误的清单。
- 每个Clip是否只有一个`反向提示词：`，并且它是最终字段、最终段落，其后无任何正文。
- `主风格 / 人物一致性 / 环境一致性 / 参考区 / 逐镜正文`是否仍散落**通用**`禁止 / 不要 / 避免`清单（写在内容所属阶段的最小内容约束不属此列）；可正向化的句子是否仍以否定形式保留。
- 反向提示词是否与正文重复、包含同义堆叠或抽象低价值词，篇幅或语义权重是否压过主体、动作、空间、摄影机、时间顺序与Handoff。
- 内容相关的最小约束是否留在它所约束的字段并保持指代清楚，而没有被抽离到末尾段导致执行歧义；末尾段是否没有反过来抢走某一阶段的局部控制。
