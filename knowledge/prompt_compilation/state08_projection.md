...
导演名、影片名、流派名、题材名、情绪标签与审美大词，例如“岩井俊二式、王家卫式、电影感、高级感、治愈感、青春感、赛博朋克、黑帮感、广告感”，统一视为上游风格意图标签或Knowledge Retrieval Label，不视为模型能够精准复现的控制参数。进入STATE-08前，每个会影响当前Clip的重要风格标签都必须先保留其剧情/情绪功能，再拆成当前Clip真正需要的具体style carriers；标签不能单独承担最终执行控制。
...
导演名或流派名如果只用于检索`knowledge/visual_styles/`与Knowledge Application Reflection，应优先留在内部记录，不默认写入最终Prompt。若具体carriers已经完整表达意图，允许省略名称以降低Semantic Trigger Pollution；若名称因用户明确要求或叙事价值仍保留，必须同时写出具体carriers。无论是否保留名称，都不得把它当作精确复现保证。

示例：`电影级真人青春短片质感，岩井俊二式青春电影氛围`不能原样结束编译。可按当前Clip事实从柔和自然光、低饱和偏暖色调、轻微胶片颗粒、自然肤质、浅景深/背景高光轻微溢出、克制运镜、停顿/呼吸/细微眼神变化中选择必要的3—5项（或更少）。这只是候选拆解，不是固定模板；不得每次整段复制，也不得无依据自动加入校园、校服、樱花、海边、夏日奔跑或其他默认青春场景包。
...
- 导演名、流派名、题材名、情绪标签与“电影感 / 高级感 / 治愈感 / 青春感 / 宿命感”等审美大词是否完成Style Label Decomposition；重要标签是否由当前Clip必要的3—5个（或更少）高价值Lighting / Color / Optics / Camera / Texture / Composition / Performance / Rhythm / Atmosphere carriers承担，而不是孤立存在或机械填满。
- 风格标签是否无依据带入当前Clip不需要的默认场景、服装、道具、天气、美术、光源或人物动作；导演名若只作检索是否留在内部，具体carriers已足够时名称是否已作为冗余删除。
...
- 风格标签与carriers是否已压缩为不抢占主体、动作、空间、时间顺序、摄影机路径和状态承接控制位置的最小集合；是否不存在Style Stack Pollution。
