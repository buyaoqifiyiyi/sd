...
- 有明确人物目标、阻力或关系变化时，当前可见行为能读出人物正在采取的策略，且策略变化有对应的Beat证据；没有新增剧情或强行升级。
- 已确认的手头事务、人物距离或地位变化只在需要时作为表演证据，并且不覆盖Action / Blocking的道具、位置、朝向或路线事实。

### Risk-driven Prompt Evidence

- 仅当当前镜头存在相应风险时，首帧主体、多人左右/前后关系、视线、朝向、地标接触、交接道具的持有者/左右手、或关键物理因果在既有字段中有清楚正向证据。
- 不存在该风险时，不因QA新增泛化锁、独立负向清单、伪精确数值或内部检查语言。
- 风险证据与Confirmed Spatial Blocking、资产锁、Camera Decision及物理事实一致；冲突时返回对应owner，而不是由Prompt临时调和。
