...
### External Prompt Reference Gate

当用户提供一份**外部文字提示词**（他人发布的教程、范例Prompt、或用户自己收集的成品Prompt）时，它属于与截图同级的**风格研究来源**，不是规则、不是Template、不是已确认事实。处理顺序固定为：

- 先按`Reference-To-System Evidence Gate`的三类划分整理：其中**可观察的可见结果**（光源方向、材质响应、色彩面积关系、构图层次、动作节奏与失败规避项）属于Observable Reference Evidence；为当前剧本提出的设计属于Project Proposal；**具名作品、机型镜头滤镜型号、平台或模型的参数与能力数值、他人的工作流步骤**一律属于Unknown / Not Transferable，不得因"参考里有"而自动继承。
- 再把Observable部分**反编译为STATE-04的Aesthetic Decision Lock四项**（反差与光比结构、色彩对抗关系、构图主张、视觉母题与变化轨迹），并按既定纪律写出选择与它排除的可见结果。外部Prompt的原文、句式、字段名与排版**不得**复制进`主风格：`或任何最终Prompt字段；最终Prompt只出现本项目自己的可执行语义。
- 外部Prompt不得改变Production-Locked Script、Canonical资产、Director Intent、媒介形式、Voice opt-in或视频Prompt永久无BGM规则；它与本项目规则冲突时以本项目规则为准。
- 外部Prompt中的模型专属写法（例如某一视频模型的时间线或参考槽位用法）只有在所选Target Model的Adapter确有该能力、且已按其能力数值复查后才可使用；不得据此写入未验证的字段、能力数值或平台参数。

用户明确要求"照这个风格做"时，只表示它进入上述反编译流程，不代表它获得了资产、规则或Schema权威。

### Temporal Reference Decode｜参考片时间轴解码

当参考材料是**视频**（而非仅截图或文字）时，先执行一次时间轴解码，再进入`Reference-To-System Evidence Gate`的三类划分。本小节不新建模块，只定义读取顺序与判定纪律；产物仍是该Gate的三类，不产生第二套Schema。

#### Measured Boundary And Motion｜边界与运动量实测

**镜头边界是量出来的，不是看出来的。** 模型看视频最不可靠的一项就是报时间：同一段画面，「这个镜头大约 3 秒」与「2.97 秒」差的不是精度，是这份表能不能用于下一阶段。因此本节划一条死线：

| 由谁定 | 什么 | 怎么定 |
|---|---|---|
| **代码** | 切点、时长、片长、帧率 | ffmpeg 场景检测 + ffprobe，两位小数 |
| **代码** | 每镜的实测运动量 | 逐帧差分曲线的区间中位数（两端剔除），避开切点尖峰 |
| **模型** | 景别、类别、运镜、画面、节奏 | 看关键帧判断——**只有这几件事是模型的活** |
| **代码** | 判断对不对 | `validate` 的确定性质量门逐条对账 |

**测量命令**（两个命令与本节数值构成机器侧的完整定义；执行步骤见`workflows/22_reference_film_study_workflow.md`）：

```text
切点    ffmpeg -i <video> -an -vf "scale=320:-2,select='gt(scene,<threshold>)',metadata=print:file=-"
运动曲线 ffmpeg -i <video> -an -vf "fps=<hz>,scale=64:36,tblend=all_mode=difference,signalstats,metadata=print:file=-:key=lavfi.signalstats.YAVG"
```

运动曲线的每个采样点是相邻采样帧的**平均绝对亮度差**（`signalstats` 的 `YAVG`，0–255 量纲，取一位小数）；每镜的 `motion` 是它在该区间内的**中位数**，且**两端各剔除一段**——切点那一帧的差分必然爆表，不剔掉的话每个镜头看起来都在动。剔除宽度按镜长自适应（`min(0.4s, max(0.1s, 镜长 × 15%))`）。区间内没有采样点时该镜的运动量为空值，不按 0 计。

| 参数 | 默认 | 含义 |
|---|---|---|
| `sceneThreshold` | 0.3 | 场景检测阈值，**越低切得越碎**；暗戏、慢片、同机位对话多的片子往下调 |
| `minShotSeconds` | 0.3 | 短于它的片段并进上一镜（闪帧、字幕跳变、转场中间帧） |
| `staticMaxMotion` | 1.5 | 实测运动量低于它 = 画面几乎没动 |
| `busyMinMotion` | 12 | 高于它 = 画面动得厉害 |
| `motionGateMinSeconds` | 1 | **短于它的镜头不查运动量门**（采样点太少，值照给） |
| `trackHz` | 5 | 运动曲线采样率 |

**景别词表与既有规范景别的对应**（左边是测量产物，右边是`knowledge/camera_language/lens_language/framing_and_scale.md`的规范景别；两边同义，不构成第二套景别体系）：

| 测量词表 `size` | 规范景别 | 测量词表 `size` | 规范景别 |
|---|---|---|---|
| `extreme-wide` | 大全景 | `medium-close` | 中近景 |
| `wide` | 全景 | `close` | 近景 |
| `medium-wide` | 中远景（全景与中景之间，规范景别无独立档，按环境占比在两者间取一） | `extreme-close` | 大特写 |
| `medium` | 中景 | `none` | 不适用（黑场、纯字卡、纯图形——画框里没有被取景的空间） |

**运镜词表的实测档与验收**：测量词表给每个运镜标一个像素上必然留下的痕迹档——`strong`（整幅画面必然移动）、`still`（机位不动）、`subtle`（变化太小或太局部，实测分不开）。据此只有一条硬门：

- **`strong` 声称却实测接近 0 → 拦。** 「声称推/拉/摇/移/跟，实测帧间变化接近 0」是拉片最常见的幻觉，这一向直接判失败，必须回看该镜重判。
- **`still` 声称而实测偏高 → 不拦，只出提示。** 固定机位前面有人跳舞，帧间差一样会爆；这是需要人回看一眼的地方，不是错误。
- **`subtle` 不设门**，数值只给人看。

**实测运动量不区分机位动还是主体动**，所以只能拦上述一向；反向只出提示。两端都判不准时回看该镜的起手帧与收尾帧：同格前后对照，取景变没变一眼就知道。

**边界两侧的常见检测错误**：叠化与暗场对暗场**漏刀**（起手帧和收尾帧根本是两个场景，就是漏刀的铁证），手持晃动和闪光**多刀**。补刀与并刀必须显式声明为人工边界，**自己加的刀不声明就过不了边界门**；被拆被并的镜头，原标注清空并写明出身——这两半是不是一回事必须重新看画面，不许把旧描述顺下去。

**这一层是可选加固，不是前置条件。** 运行环境没有 node 或 ffmpeg 时，按`### 结论分层与不可确认项`与`### 硬性识别规则`执行一次**纯人工拉片**：逐段回看并按画面读数记录时间码，结论同样必须过上述运镜验收——**声称大动却看不出画面位移的，一律降级为不可确认**。不得因为无法实测就把「看起来像推镜」写成推镜。
...
```text
完整观看 → 检测镜头切点 → 按时间码建立Shot段落 → 检查每镜首帧/中段/末帧
→ 区分剪辑换位与镜头内运动 → 分析人物/道具/环境的运动贡献
→ 提炼跨镜头的机位递进 → 分类为可观察证据 / 项目提案 / 不可确认项
```

**每镜至少判断**：起始与结束景别是否相同；摄影机空间位置是否连续改变；画面变化来自摄影机还是演员；是否只有焦点变化；是否只是剪辑切到了另一个机位；运动由什么动作或信息触发；最终停在什么构图；下一镜为什么在这里切入。

**硬性识别规则**：景别突然变近、观察方向突然改变或出现反打时，**默认先判定为剪辑换机位**；只有连续镜头内部存在可见空间位移、视轴转动或光学变化时，才记为运镜。不得把“中景切近景”记成Push In。

**结论必须分层**：可见证据（可直接从画面读出的机位、景别、运动、切点与信息顺序）、推断（对该运动触发与意图的解释，须显式标明为推断）、不可确认（器材、轨道/稳定器、拍摄顺序、幕后流程、参数与后期）。第三类不得因“成片看起来如此”而当作已确认事实。
