...
1. **视觉连续 ≠ 剧情连续。** 先分类，再决定上一Clip尾帧是否是正式生成参考。只有`视觉连续`才强制引用上一Clip尾帧；`剧情连续`或`主动切场 / 切世界`不得机械引用。
...
3. 去重后，仅在`视觉连续`的Direct / Reference-Only分类下加入上一Clip尾帧；`剧情连续`或`主动切场 / 切世界`不得预留该尾帧图片位。
...
- 五个Acceptance Scenarios与Three Global High-Priority Rules可被静态检索。
