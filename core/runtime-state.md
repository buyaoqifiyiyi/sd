...
每次路由必须取得 `CURRENT_STAGE`、`CONFIRMED_STATE`、`ACTIVE_MODULE`、`CURRENT_CLIP`（适用时）、`SELECTED_IMAGE_MODEL`（STATE-03新建或重编Image Prompt前，供资产Prompt编译消费）、`SELECTED_MODEL`（STATE-06 完成后的 Model Selection 成功后，供 STATE-07/08 消费）及 `OUTPUT_TYPE`。这些是状态合同中的路由视图，不是第二套 Schema。
