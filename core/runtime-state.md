...
每次路由必须取得 `CURRENT_STAGE`、`CONFIRMED_STATE`、`ACTIVE_MODULE`、`CURRENT_CLIP`（适用时）、`PROJECT_IMAGE_MODEL_DEFAULT`、`PROJECT_VIDEO_MODEL_PREFERENCE`、`SELECTED_IMAGE_MODEL`（STATE-03新建或重编Image Prompt前，供资产Prompt编译消费）和`SELECTED_MODEL`（STATE-06后经当前Clip能力复核成功，供STATE-07/08消费）及 `OUTPUT_TYPE`。这些是状态合同中的路由视图，不是第二套 Schema。
