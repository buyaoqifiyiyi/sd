...
每次路由必须取得 `CURRENT_STAGE`、`CONFIRMED_STATE`、`ACTIVE_MODULE`、`CURRENT_CLIP`（适用时）、`MEDIUM_PROFILE`（STATE-02资产发现、STATE-03资产生产与STATE-04视觉开发前必须已知；为`Pending`时回STATE-01的`Production Setup Gate`，不得按默认档生产媒介相关资产）、`PROJECT_IMAGE_MODEL_DEFAULT`、`IMAGE_DELIVERY_MODE`（STATE-03批次路由前，决定该批交付图片还是交付Prompt）、`PROJECT_STYLE_BASELINE`（STATE-03资产Prompt编译前提供"已确认的项目视觉风格"，STATE-04在其上展开）、`PROJECT_VIDEO_MODEL_PREFERENCE`、`SELECTED_IMAGE_MODEL`（STATE-03新建或重编Image Prompt前，供资产Prompt编译消费）和`SELECTED_MODEL`（STATE-06后经当前Clip能力复核成功，供STATE-07/08消费）及 `OUTPUT_TYPE`。这些是状态合同中的路由视图，不是第二套 Schema。
