...
SD Film现在默认由`Director Module / Director Intelligence Layer`贯穿剧本→视觉→场景→镜头→Clip→Prompt→Editing→Review。它从“这段戏为什么存在、观众先知道/感受什么、人物关系怎样变化”开始，再让镜头语言承担构图、景别、机位、焦段/距离、前中后景、遮挡/揭示、运镜触发与Hold / Cut节奏。每个镜头都先写清“观众从什么状态、经什么触发、变到什么感受与落点”，再指定唯一主要的画面关系承担项；运镜必须给出触发、路径中的构图/空间/焦点变化、停止点与运动理由，并说明观众是先知、同时知还是后知。只写情绪标签或“电影感”不成立：每个镜头还要通过“换成平视中景 / 固定机位后观众会失去什么”的检验；复杂的环绕、穿行或连续越轴每个Clip最多破例一次，且必须绑在该Clip的视觉高潮上。最终Seedance Prompt仍使用原有模板，不会多出一整套导演分析字段。
...
**自动模式只是自动确认，不减少流程与产物。** 该跑的阶段、Required Read、QA、Completion Gate、状态写回一件不少；该给的东西——分镜表、Clip表、最终提示词、生产交付包——按各自的完整模板照给。它省掉的只是"等你点一下确认"的往返。如果哪次它用一句"已完成场景、镜头与执行规划"顶替了完整工件，那是违规，可以直接说：`给出交付收据，把缺失的完整工件补齐再继续`。最终提示词那一轮的收据里还会多一项`Prompt纪律自检`：六条逐条写`过 / 不过 / 不适用`并附证据。它的作用只是让你看见哪一条没做到，**不会**让原本能交付的提示词判不过，也**不会**把自检文字写进提示词本身。
...
**你没提异议就是认可。** 只要某一批已经完整展示给你、逐项能核对到对象和版本，你看过之后说“继续 / 下一步 / 好的”，就按既有语义算整批通过——不会再多要一次“确认 / 批准”。反过来，**你从没看到过的东西**（没有机会提异议）、以及范围含混对不上号的东西，不会因为没人反对就被算作认可。**认可与“此刻是对话里的文字还是磁盘上的文件”无关**：你点头过的内容就是正式工件，不会因为还没写成文件而被降级、被当成未确认，或被要求重新确认一次；系统顶多在打包前把它原样落盘写成文件。系统还会在 `00_INDEX.md` 里逐项列出**没进包的东西和原因**（从未展示 / 未确认 / 已否决 / 缺确认记录），不会静默丢掉。
...
**包里的三类工件必须是完整版，不能是摘要。** `04_scenes` / `05_shots` / `06_clips` 装的是完整 Scene Breakdown、默认 5 列分镜表与默认 6 列 Clip 表；**"SHOT-001 女孩按灭手机；SHOT-002 她走向楼梯"这样一行一句的清单不是分镜表**，两条 bullet 也不是 Scene Breakdown。打包时会先跑 `scripts/validate_delivery_artifacts.py` 对应的三类检查，**任一工件是摘要或不合格就直接拒绝出包**（不生成包目录、不生成 zip），并逐条告诉你缺的是哪一类、哪个文件、缺什么——这时补全那一件再重新打包即可，不要接受"包已生成"的说法。某个类别确实没有已认可内容时，该目录里会有一份 `NOT_APPLICABLE.md` 写明依据，而不是留一个空目录让你猜。

包内固定八类：`01_script`（锁定剧本与剧本分析）、`02_assets`（资产登记表 + 每条已确认资产的图片，按 CHAR / ENV / PROP / FX / Support Board）、`03_visual_development`（Project Bible、已建立的视觉方向与 Aesthetic Decision Lock；视觉开发默认不单独向你展示确认，因此这一类常常是 `Not Applicable`——那是打包入选口径，**不代表这一步被跳过**）、`04_scenes`（已确认的场景拆解与适用 Sequence）、`05_shots`（已确认的专业分镜）、`06_clips`（已确认 Clip 表）、`07_references`（**实际投喂给模型的非资产参考**：已确认草图 `REF-SKETCH`、上/下镜尾帧 `REF-TAIL`、色卡、你提供的合法首尾帧——它们和提示词里的参考条目一一对应）、`08_design`（**只给系统和你自己看的内部参考**：调度图 Top-down Blocking Map、你要求过的 Storyboard、Look Frame 试片帧）。每类都有清单文件，包里另有一份 `00_MANIFEST.md` 记录每份文件的来源、字节数、校验和与接受依据。

**07 和 08 的区别只有一条：投不投喂给模型。** 你说得对——草图和调度图都是"为了参考"才产生的；区别是参考给谁。俯视空间关系**确实会进参考位**，只是它进参考位的合法载体是 `REF-SKETCH` 里那块 Spatial Proof 区域（一张工程版式单图），而不是调度图本身：多画面、带机位锥和箭头标注的规划图直接投喂，会把线条、边框、标注、多画面结构带进生成画面。所以调度图进 `08_design` 归档，但不占参考位、不进提示词的参考字段。

**包是 Clip 表确认那一刻的快照。** 提示词撰写阶段（STATE-08）之后才生成的调度草图 `REF-SKETCH` 和上/下镜尾帧 `REF-TAIL`，**不会让包失效，也不要求系统重发整包**——系统只会把它们的位置与规范文件名列给你，你想放进 `07_references/` 就自行放。想让它按现在的状态重出一次，说一句「重新打包」即可。
...
**如果这个项目此前从没保存成文件**，系统会在打包前**先把已确认的内容原样写成项目文件**（锁定剧本、场次拆解、分镜表、Clip 表、资产登记表），再生成包与 zip。这一步只是落盘：不新增确认、不改写你已认可的内容、不重做已完成阶段；它也不是"凭空补文件"——包里的每一份仍然必须同时有确认记录和真实文件。只有环境确实不能写文件，或某一项根本没有确认记录时，才会降级成清单形态或逐项报告。

包建在项目目录**之外**：`<项目目录的上一级>\<项目ID>_packages\<版本>\`。**包本体是一个目录，旁边的 zip 是同一份包的压缩形态**——内容一模一样，不是两份东西；zip 只是方便你整体拿走。包只**复制**已确认文件，不改内容、不改名项目里的原图、不动项目状态。

**每个类别清单一行一个文件，并带可读列**：`文件名｜Asset ID｜资产名｜Purpose｜View角色｜版本｜Prompt引用名`。所以 `ENV-002｜Layout_ENV-03.png` 在清单里会写成"环境资产②的 Lateral 侧向图"，并给出它在提示词里的引用名——**包内文件和提示词参考条目可以逐行对着看**。同一个 Asset ID 的多张图（环境四视角、道具 Identity / State）必须各占一行，不会挤在一格里；**文件名本身不变**，已确认资产不追溯改名。

资产图片的文件名在**图片确认那一刻**就固定下来，形如 `CHAR-001｜Identity.png`；环境多视角写作 `ENV-002｜Layout_ENV-01.png`（View ID 沿用 ENV-01 母参考、ENV-02 反打、ENV-03 侧向、ENV-04 俯视校验）——同一个环境空间的各个视角属于**同一个环境资产**、共用这一个 Asset ID，不会为每个视角另开一个资产号；Support Board 写作 `BOARD-CHAR-001｜A-01.png`。最终视频提示词里每个真正要投喂的参考资产都会写出它的稳定引用名 `CHAR-001｜林秀兰`（环境多视角在后面补 View Code，如 `ENV-002｜面馆主视图_ENV-01`），以 Asset ID 为键与包内文件名一一对应，所以“提示词里提到了哪张图”和“包里有没有这张图”可以逐条对上。提示词会放在一个**可独立复制的纯文本代码块**里给你，`@图片1` 这类上传顺序标记不会因宿主渲染而在复制时失真。系统会在打包时做这个对应检查：提示词引用了包里没有的文件、只写了名称或中文标签而没写资产 ID、或同一张图被当成两个资产，都会直接报出来，不会静默放过。
...
**你可以点名任何一个辅助模块或任何一个主阶段单独跑，不必走完整流程。** 两种形态：

- **只调用辅助模块**：Storyboard、空间走位、角色音色（AUDIO）、配乐（MUSIC）、海报封面、Sequence 规划、局部剪辑、系列管理、参考片拉片——它们本来就不占主流程阶段。
- **只调用一个主阶段**：STATE-01 到 STATE-08 里的任意一个，例如"只重做分镜表""只输出 CLIP-003 的提示词""只做 CHAR-001 的角色资产"。

可以直接这样说：

```text
调用sd，这次只做 STATE-06 分镜表，别推进后面的阶段。
调用sd，只调用 Storyboard 模块，出一版分镜图。
调用sd，只重做 STATE-05 的场景拆解，其他已确认的内容不要动。
```

三条规矩，先说清楚免得踩坑：

1. **"单独跑"不等于"跳过前置"。** 被点名的那个阶段仍然需要它自己的输入：要么项目里已经有已确认的上游工件，要么你在这一轮明确给出可核验的输入。缺什么，系统就停下来告诉你缺哪一件、该由谁产出，**不会用编造的内容顶替**（编造只发生在"空跑演练"里，而空跑不产出任何交付物）。
2. **独立跑出来的东西是正式工件，但不计入项目进度。** 交付物按完整模板给你，你确认后照常登记版本、照常进生产交付包、照常被后面的阶段复用；但**它不会被算作"这个阶段在主流程里完成了"**——`Completed States` 与"下一步"都不因此前进。等你按顺序真正走到那一步时，系统会直接**沿用**这份已确认的工件，而不是让你再做一遍。
3. **已经确认过的东西要重做，属于返修。** 对已确认且没受影响的工件做独立调用，会走变更流程生成新 Revision，不静默覆盖旧版本。

想要的是"从我点到的地方往后一路做下去"时，那就不是独立调用——直接说"继续 / 下一步"即可，那是推进命令，会按主流程走并正常计入进度。
...
7. 你可以上传色卡并明确“将此色卡作为项目色彩基线”。它会作为`Project Color Reference`进入视觉开发，而不是角色、环境、道具或效果资产；只有真实图片已可访问、你已确认采用、且某个Clip确有光色漂移风险时，才会作为该Clip的受控模型图片输入。同样，你也可以丢来一份你喜欢的外部提示词（别人的教程、范例Prompt、你自己收集的成品Prompt）并说“照这个风格做”：系统会把它当作与截图同级的**风格研究来源**，先分出可直接观察的可见结果、为当前剧本提出的提案，以及不可迁移的部分（具名作品、器材型号、模型参数与能力数值、他人的工作流步骤），再把可观察部分反编译成视觉开发的美学决定。**它的原文、句式和字段名不会进入你的最终视频Prompt**，也不会成为规则、资产或Prompt Schema；与项目规则冲突时以项目规则为准，同一份外部提示词也不会自动变成某个Clip的模型输入参考。
...
- **媒介形式**：`live_action`（真人 / 实拍）、`3d_animation`（三维 / 三渲二 / CG）、`2d_anime`（二维 / 漫剧 / 手绘 / 动态漫画）。它不改变题材，但会改变镜头、表演与美学三套语言——尤其是 2D 漫剧不能沿用实拍焦段、光比与器材参数。它在剧本定稿后的`Production Setup`确认单里和模型、风格一起问一次，**早于资产制作**；你在指令里已经写明媒介时不会重复问。确认前系统不会按默认档出资产：2D 与实拍的资产结构与材质语言不通用，答错档会让已出的资产作废。2D 档另有一套：角色资产是**设定集与画风锚**（角色设定区 + 表情区 + 画风与色指定区），不套用真人/三维的五区资产图；镜头语言改走绘制媒介的等效表达——版面放大代替推、版面向移代替摇、关键帧密度与冲击帧代替帧率与运动模糊，实拍焦段、光圈、真实景深与器材参数在该档一律不写。
- **受众定位**：你在项目里写明受众（学龄前 / 儿童家庭向 / 通用）后，系统会按它调整三件事——什么内容可以出现、以什么方式出现（暴力、恐惧、危险动作、成瘾物的边界，以及**可模仿性**：受众越小，危险动作越必须在同一段里给出可理解的负面后果），以及理解难度与表演、声音尺度。**它不会替你猜**：媒介是动画、类型是冒险、平台是少儿频道、画风可爱、片子很短——都不算依据；"动画片是给孩子看的"是最常见的一种错猜。你没写就按通用规则走，也不会被登记成儿童向。另外，儿童向**不等于降智**：可理解性要求不会去简化因果或删掉冲突；各国各平台的分级条文属于外部事实，得由你提供来源，系统不会自己编一套或声称"符合"某个分级。
- **纪实 / 非虚构**：目标确认为纪录片、观察式、访谈式、档案重组或口述史时，系统按纪实纪律执行：每条事实陈述都要能指向可核对来源（内部维护一张"主张 → 来源"台账，答不出来源的不能当事实说）；重现段落必须可区分并标注依据，不做无依据的重现、不替人物读心；旁白与字幕只讲可核对的内容，引述标明是引述；时间线与事件顺序必须与来源一致，压缩不能制造虚构因果。**最要紧的一条**：AI 生成的画面不会被呈现成档案或真实影像——不配纪实字幕、时间码或档案式包装来让它"像史料"；涉及真实人物、机构与事件时，授权必须由你提供。它和短剧、品牌片三套规则互不套用。
- **品牌与商业片**：项目里确认了品牌诉求时，系统按它把**一条**注意焦点链排出来——产品在画面里承担什么角色（主角型 / 使能型 / 背景型）、可信度靠什么建立（材质、真实光源、使用痕迹）、利益点用可见结果而不是形容词表达；品牌调性走已有的视觉语法基线，不另起一套。**商业事实一律不由制作生成**：真实价格、SKU、Logo 文字、功效与资质、受监管承诺、免责声明、授权人物与声音都必须由你提供，需要文字级正确的元素默认留给后期叠加；缺口按待确认记录，不编造补齐。它也不会把普通叙事项目当广告做——你没给品牌诉求就不加载，时长短、平台像短视频都不算数。品牌片不享受例外：资产确认、Completion Gate、参考预算、无配乐规则全部照旧。
- **商业受众与形态**：确认品牌诉求后，系统按**受众角色**排说服路径（决策者看结果与代价、使用者看过程体感、影响者要一句能转述的可见事实、代决策者看安全与后果），一条主链、其余由次级承载；并按**形态**确定内容义务（宣传 / 科普 / 产品 / 案例 / 招商 / 雇主品牌六类），形态不决定段落数与秒数。**科普多守一条**：知识要由可见过程承载、"懂了"要表现为可见变化，医学、健康、法律、金融、安全、工程等**专业结论一律由你提供**，不推断、不为好读简化到失真。受众是儿童向时，年龄轴的适宜性优先。
- **发布平台**：写明交付渠道后，系统按它调整**注意窗口**（主要矛盾得在观众离开前进入）、**完播与结尾**（单条独立 / 循环 / 连续追看，决定收束还是留钩子）与**转化落点**（关注、私信、留资这类结尾动作必须由可见行为收束，并回答"前面哪个铺垫让它可信"，不能只靠文字落版）。**平台事实不由制作编**：推荐机制、时长上限、尺寸数值、投放要求都得你提供。**它不会替你猜平台**——媒介是动画、题材像广告、片子很短都不算依据。平台与媒介、类型、受众、时代地域是五条独立的轴。
- **时代与地域**：项目的**时代背景**与**主要地点**写进项目后，系统会按它约束画面上的可见事实——什么技术、器物、服装形制、字体标识、照明条件是那个时代允许的，夜景能用什么光源，称谓与身体距离是否符合该地域。它**不会替你推定**：平台、题材、导演风格、参考片都不算依据，你没写就不猜；**现代题材同样要确认**，当代项目的约束是"不出现与当下不一致的东西"。所有结论分三类：已确认来源、合理推断（会标明是推断）、不可确认（具体年号、真实机构、真实人物、真实品牌一律不写成事实）。它也不等于风格——用某个导演不等于把项目改成古装，那是另一条纪律。
- **交付画幅**：横屏还是竖屏由你说了算——你在请求里写`9:16`/竖屏，或项目已确认交付规格写明竖屏时，系统按竖屏构图执行：双人改前后或上下错位、群体改纵深排列、横向长距离运动改纵向或朝向镜头、面部与关键信息避开顶部与底部的平台安全边带。**它不会替你推定**：平台是短视频、目标是短剧、素材是竖屏参考都不算数，你没说就不猜。横屏画面也不会被裁成竖屏交付。
- **类型片**：悬疑/惊悚、动作、爱情、喜剧、恐怖、犯罪六类各有一套**呈现层**倾向（镜头、表演、声音、节奏各自在什么条件下成立），在视觉开发（STATE-04）时按你登记的类型落进 `Visual Grammar Baseline`，之后 STATE-05 到 STATE-08 沿用这套基线，不会在 Prompt 阶段另起一套。它在项目里登记了类型时才加载——**你没写类型时系统不会替你猜**，也不会从媒介、平台、题材标签或参考片反推。类型倾向是**带条件的候选手段**，不是配方：不会出现"第几分钟必须反转""每分钟几个笑点"这类固定套路，也不改写剧情、人物与已确认资产。想指定就说`这是一部悬疑片`，或直接写进项目需求；类型与媒介、导演风格是三条独立的轴，互不覆盖。
...
- 角色有对白不会自动触发音色模块。
- **资产图画幅**：人物资产图默认 **9:16 竖版**（单角色画布；五区版式不变，不会为凑比例挤版）。这是默认值——你明确说“横版 / 用 16:9”时按你说的走。
...
- **资产图画幅**：环境资产图默认 **16:9 横版**，每个视角各自一张；你明确指定其他比例时以你为准。
...
- **资产图画幅**：道具资产图默认 **16:9 横版**（1×4 横版设定图，四格等宽）。
...
- **资产图画幅**：正式FX资产图默认 **16:9 横版**；Support Board（含角色参考板）同样按横版多对象版式。
...
- 输出分辨率与宽高比由你在生成时自行选择，**不出现在提示词里**；提示词也不声明标题与时长，这些生成参数属于Clip Plan。分镜或阶段的数量限制写在末尾`反向提示词：`（Seedance 2.0 与 MiniMax H3）或由`时间线：`声明（Seedance 2.5），并与实际数量严格一致；模型有扩写倾向时，末尾会补一句同义兜底。这是既有字段的内容要求，不会新增任何 Prompt 栏目。
...
- 雨声、风声、纸张、脚步、道具或乐器声只进入各分镜`音效`。正文以正向、可执行的目标状态为主：能写成“表演克制、左手持续持伞”等正向要求时，不在`主风格`、一致性或各分镜里反复堆“禁止 / 不要 / 避免”。**负向控制写在它约束的内容所属的阶段**——跟具体动作、空间、道具、表演、声音或字幕有关的约束写在那个阶段的字段里，跟着情节一起读；只有真正跨阶段、放不进任何单一阶段的通用禁止项才合并到每个Clip末尾唯一的`反向提示词：`，该段固定以禁背景音乐规则开头并承担阶段数兜底，后面不再追加正文。历史事故物、其他Clip状态与未来剧情会删除，同义错误合并为少量风险类别。
- 阶段内部的写法也是固定的：`画面与镜头`先给镜头号，焦段与光圈只在真的决定景深、透视或对焦时才写，并紧跟它换来的可见结果；`人物动作与情绪`在表演重的阶段按八项编号逐条写，维度名用`｜`分隔，写成`时间：`属于格式错误。
...
- 交付 Prompt 就是本轮的终点：给出 Prompt 与生产交付包后直接收尾，不会追问“要不要我替你提交 / 用哪个入口 / 会不会消耗额度”。要实际提交时，明确说一句“实际提交”或“代为提交”并说明入口（Dreamina 网页端或方舟 API）即可。
...
服务本项目的宣传封面已由 Poster Design Workflow 的**封面系 Lane** 正式支持：平台封面、剧集/短剧封面、竖屏封面、社媒宣传封面、视频封面（缩略图/封面图）与频道主视觉。封面与电影海报共享同一管线和质检门，但信息层级以秒级识别优先：大字标题在缩略图尺寸可读、主体与对比优先。

推荐这样说：

```text
调用sd，根据当前项目为短剧设计一张平台竖屏封面，沿用已确认角色资产，不重新设计；标题“……”在缩略图下可读。
```

平台封面的尺寸、安全区与导出格式以**渠道官方规范或你提供的规格**为准；你没给规格时会标为待确认，不会凭记忆硬编码平台数值。
...
- 与影片内容无关的通用平面设计（头像、壁纸、信息长图）不在射程内，会如实说明。
...
- 正式支持电影海报、Key Art、One-sheet、先导 / 正式 / 角色 / 概念海报、标题字方向与项目宣传封面（平台 / 剧集 / 竖屏 / 社媒 / 视频封面）。
...
服务本项目影片宣传的社媒封面和视频缩略图已由封面系 Lane 支持：

```text
调用sd，把当前电影海报主视觉重排为16:9横版平台Key Art，并派生一张1:1定档宣布封面。
```

```text
调用sd，根据当前项目做一张YouTube视频封面，沿用现有资产，大字标题在缩略图尺寸可读。
```

与影片内容无关的通用平面设计（个人账号缩略图、直播封面、信息流卡片、头像、壁纸、信息长图）仍不在射程内；这类请求会如实说明，不硬写不存在的流程。
...
```text
拉片这条视频：先按时间码建立Shot段落，区分剪辑换机位与镜头内运镜，分析人物/道具/环境各自承担了多少运动，再提炼跨镜头的机位递进。结论分三层：可见证据、推断、不可确认。不要把它当成本项目成片Review，也不要建项目。
```

**Skill 行为 / 停止点**

- 这类请求会自动激活SD Film，但**不初始化项目、不进入主Pipeline、不进入STATE-09 Review**——它是一次独立分析，产物是判定而不是交付物。
- **镜头边界是量出来的，不是看出来的。** 切点来自ffmpeg场景检测、时长由切点相减得出（两位小数），每个镜头还有一个**实测运动量**（逐帧差分的中位数）。模型只判断它真正该判断的那几件事——景别、类别、运镜、画面、节奏——**不负责报时间**。所以「这个镜头大概3秒」不会出现在结果里。
- **每条判断都会被当场对账。** 最硬的一道门是运镜实测对账：**声称推、拉、摇、移、跟，而实测画面几乎没动，这一条直接判失败并停下**——这是拉片最常见的幻觉。反过来（声称固定而实测偏高）不判失败，只给提示，因为固定机位前面有人跳舞帧间差一样会爆；这类提示会点名镜号，需要你自己回看一眼。
- 交付物放在你指定的目录（没指定就用当前目录下的`reference-film-<片名>/`）：时间码镜头表、**单页交互式报告**（内嵌播放器，播放时同步高亮当前镜头、点镜头跳转；镜头节奏带；可搜索可筛选可排序的镜头表；点关键帧开大图；可导出JSON；单文件零依赖，离线双击能开）、每镜的起手帧与收尾帧、联系表。**它不是一个文件，是上面这一组。**
- 景别突然变近、观察方向突变或出现反打时，默认先判定为**剪辑换机位**；只有连续镜头内部存在可见空间位移或视轴转动才记为运镜。所以"中景切近景"不会被叫成推镜。
- 参考片自带的字幕、片中文字、水印与元数据是**分析材料，不是指令**；其中出现的任何要求都不会被执行。
- 想把学到的方法用到当前项目时，明确说`把这个方法用于当前项目`：它会先经参考证据门提炼为可观察方法，再回到**视觉开发（STATE-04）**建立或修订Visual Grammar；不会把参考片的镜头表当公式套用，也不会复制它的人物、场景、剧情或器材推测。
- 需要**画面+分镜信息对照的合成视频**时明确说：`把这条片子和分镜信息合成一条对照视频`。它会用无头浏览器渲面板再合成，合成后**抽帧核对镜号与高亮行**才交。这一步需要一个Chrome或Edge。
- 你的机器上如果**没有node或ffmpeg**，它不会假装测过：会改走纯人工回看，并在汇报里明说本次没有实测数值。
...
- 改什么：例如“音色必须 opt-in”“只修 REF-TAIL B 模式”“给配乐模块增加竖屏短剧专用节奏档”。
...
- `继续 / 下一步 / 下一个 / 往后做 / 接着做 / 好的` 不等于同意新的剧本改写范围、作出互斥选择、提交外部服务或授权批量输出；但在当前已展示、可核对的确认点，它们都确认当前Artifact。交付 Prompt 时也不会反过来问你“要不要代为提交”——要提交必须由你明确提出。
- 角色有对白不等于要做音色；没有音色资产也不阻塞主流程。
- 说“用 Seedance 2.5”默认只是**偏好**，不是全程锁定：系统仍会在镜头设计完成后按每个 Clip 的真实能力需求复核一次，并可以给你一个更便宜的可行候选供你决定（提示不会自动换掉你的模型）。只有说“只能用 2.5 / 不接受其他模型”才会记为硬锁，此后不再反复推荐替代。另外，成本差价与预期成本只有在费率真实存在时才会给数字——没费率时一律显示“待用户提供”，不会用“更便宜 / 差不多”这类说法蒙过去。
...
- 电影海报、Key Art 与项目宣传封面（平台/剧集/竖屏/社媒/视频封面）当前受支持；与影片内容无关的通用平面设计没有专用模块。
- 资产图的比例不会从你给的参考图继承：人物类默认 9:16 竖版、环境/道具/FX 与 Board 类默认 16:9 横版；要别的比例直接在指令里说。
- 讨论时说到的导演、流派、题材风格或“名场面”不会被直接写进项目：这些词会先被判定是当前项目的必要身份，还是只在调用模型的默认视觉包，未确认的一律不补成设定。所以同一轮通常只给**一个主锚点**并写明代价，不会一次抛给你好几个方向；要多个方向对比就直接说。

## 维护约定

每次Skill正式修改完成后都必须执行`references/maintenance_self_check.md`中的18项`Skill Update Self-Check`、`Standalone Skill Discovery Guard`和`Unconditional Chat Runtime Startup And Recovery Guard`。即使只修改文案、Knowledge、Template或拼写，也必须运行普通Chat启动与旧项目恢复基线，以及独立Skill发现基线，避免后续优化使Chat无法正常调用、出现两份同名Skill或误要求Work。该Reference是维护QA的唯一权威来源；本说明书只说明用户可见的调用和报告方式，不复制完整检查细则。
