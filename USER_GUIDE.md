...
当资产全部确认、Clip 表也确认之后、最终视频提示词之前，系统会把这一路做出来的、**你认可过的**东西按类别打成一个包（含 zip）给你。你可以直接说：

```text
调用sd，把我确认过的生产物按类别打包给我，图片文件名要和最终视频提示词里的参考资产一一对应。
```

**包里只有你认可的。** 每一项都必须同时满足两条：有可回查的确认记录，并且真实文件存在——只满足一条的不进包。所以候选图、被拒的图、只用来做设计决策的外观参考图都不会混进去；**光是出现在资产表里、或者被后面阶段用过，不算认可**。

**你没提异议就是认可。** 只要某一批已经完整展示给你、逐项能核对到对象和版本，你看过之后说“继续 / 下一步 / 好的”，就按既有语义算整批通过——不会再多要一次“确认 / 批准”。反过来，**你从没看到过的东西**（没有机会提异议）、以及范围含混对不上号的东西，不会因为没人反对就被算作认可。系统还会在 `00_INDEX.md` 里逐项列出**没进包的东西和原因**（从未展示 / 未确认 / 已否决 / 缺确认记录），不会静默丢掉。

包内每一条都标注`Approval Basis`：`User Confirmed`（你逐项确认的）、`Confirmed (batch, no objection)`（整批展示后你没提问题）、`Auto-accepted under FAST`（你在快速模式下授权系统自动接受的），三者分开写，不合并。想只要你自己经手的，说一句「只打包我本人确认过的」即可。

包内固定六类：`01_script`（锁定剧本与剧本分析）、`02_assets`（资产登记表 + 每条已确认资产的图片，按 CHAR / ENV / PROP / FX / Support Board）、`03_visual_development`（Project Bible、已确认的视觉方向与 Aesthetic Decision Lock）、`04_scenes`（已确认的场景拆解与适用 Sequence）、`05_shots`（已确认的专业分镜与适用 Storyboard）、`06_clips`（已确认 Clip 表）。每类都有清单文件，包里另有一份 `00_MANIFEST.md` 记录每份文件的来源、字节数、校验和与接受依据。

**使用前提：需要能读写本机目录的环境**——ChatGPT 桌面应用 / Codex（本地 Work 模式）这类宿主才能真的产出包目录和 zip。判定看的是**本轮实际能力**，不是平台名字：能读项目目录、能写包位置、能压缩，才产包。普通 Chat（本机目录不可读）**不会**产 zip 或包目录，而是给你一份纯文本的类别清单、`文件名 ↔ Asset ID ↔ 用途 ↔ 版本`映射和目录骨架，并明确标注“未打包”，由你在自己环境里建目录、重命名、压缩。读写能力不完整时按实际能力降级（例如只读不能写＝给你完整清单自行落盘），但**任何情况下都不会用文件名或空目录假装包已经生成**。

包建在项目目录**之外**：`<项目目录的上一级>\<项目ID>_packages\<版本>\`，旁边的 zip 就是可以直接拿走的成品。它只**复制**已确认文件，不改内容、不改名项目里的原图、不动项目状态。

资产图片的文件名在**图片确认那一刻**就固定下来，形如 `CHAR-001｜Identity.png`；环境多视角写作 `ENV-002｜Layout_ENV-01.png`（View ID 沿用 ENV-01 母参考、ENV-02 反打、ENV-03 侧向、ENV-04 俯视校验）；Support Board 写作 `BOARD-CHAR-001｜A-01.png`。最终视频提示词里每个真正要投喂的参考资产都会写出这个文件名，所以“提示词里提到了哪张图”和“包里有没有这张图”可以逐条对上。系统会在打包时做这个对应检查：提示词引用了包里没有的文件、或同一张图被当成两个资产，都会直接报出来，不会静默放过。

如果此时还有类别没做到（例如 Clip 表未确认，或某个资产还没确认），系统会告诉你缺哪一类、缺在哪个 Gate，不会给你一个假装完整的空包。之后资产换了版本或 Clip 表重跑，这个包会标记为已失效并需要重新生成。
...
SD Film **不维护项目登记表**：不会建立、读取或更新任何项目索引、清单或注册文件。

这意味着：

- **不用先"登记项目"再用。** 你直接开一个对话说"根据这个剧本做一条片子"，它就开始干活；项目文件（如果在本地）只落在那个项目自己的文件夹里，Skill 安装目录始终是只读的通用定义。
- **不用怕漏登记。** 没有登记表，也就没有"哪个项目忘了登记"这回事。
- **找回旧项目靠路径或 Checkpoint。** 你说项目文件夹在哪、或者报项目 ID 并确认项目目录，它就能接着做；跨会话恢复也走 `重新调用sd，恢复当前项目，从最后一个安全Checkpoint继续`，不查任何索引。
- **它不会自己翻你的磁盘。** 你说做新片子，它不会去扫历史项目、也不会找"看起来像"的项目来套用。
- 项目做过多少个、分别在哪，由你自己的文件夹组织决定，与 Skill 无关。
...
| 生产交付包 | `调用sd，把所有已确认的生产物按类别打包给我，图片文件名与最终视频提示词的参考资产一一对应。` | 生产交付包（Clip表确认后、最终Prompt前的交付动作） |
...
- 空间重建为`Full`时：环境主图（母参考）先单独确认一次；确认后其余方位视图在**同一轮内一次出齐**（后一张沿用前面已生成的视图作为累积参考，避免越补越飘），整组就绪后只确认一次，不逐张停。其中某一张被挑出时，只有它和依赖它的后续视图重跑。
