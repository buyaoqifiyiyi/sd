...
包内固定六类：`01_script`（锁定剧本与剧本分析）、`02_assets`（资产登记表 + 每条已确认资产的图片，按 CHAR / ENV / PROP / FX / Support Board）、`03_visual_development`（Project Bible、已建立的视觉方向与 Aesthetic Decision Lock；视觉开发默认不单独向你展示确认，因此这一类常常是 `Not Applicable`——那是打包入选口径，**不代表这一步被跳过**）、`04_scenes`（已确认的场景拆解与适用 Sequence）、`05_shots`（已确认的专业分镜与适用 Storyboard）、`06_clips`（已确认 Clip 表）。每类都有清单文件，包里另有一份 `00_MANIFEST.md` 记录每份文件的来源、字节数、校验和与接受依据。
