...
它仍会在首次视频模型选择、缺失必要输入，以及真实人物/品牌/法务内容、外部服务或实际视频Review等需要选择或额外授权的情况停下。制作版剧本锁定、资产候选图片转为正式Canonical资产、Clip Plan等已展示确认点可直接用推进表达确认。随时说`关闭自动推进`即可恢复标准模式。
...
8. Seedance 2.0为4—15秒。Seedance 2.5为4—30秒：实际生成秒数由你在该窗口内选择；4—15秒沿用稳定`Standard Clip`，16—30秒由目标时长自动进入内部严格预检，不要求你额外选择`Long-form Clip`。未知网关状态不会自动限制为15秒；若实际提交被平台拒绝，系统才返回Clip规划做最小调整。2.5现在使用独立的多模态时间线Prompt模板，默认按30图、10视频、10音频、合计50项（视频/音频各自总时长≤30秒）的能力上限规划；它会按当前Clip实际需要少用，但不再人为回退成9图上限。每项参考仍必须有唯一用途。纯音频驱动动作、口型或节奏也必须由你明确指定。
9. 只有你明确说“使用Dreamina网页端”时，系统才会按该入口准备30—180秒一键长视频、标注编辑、绿幕、双视频转场或多格分镜的提交方案；这些不会被误报为方舟/API默认能力。
9. Clip Plan、分镜或其他需要确认的生产成果，仍会在可核对的当前版本展示后才标为Confirmed；你用任一推进表达即可完成该确认。
