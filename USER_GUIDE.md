...
- 会做 Prompt Pollution 清理：合并重复、消解冲突，把导演名、流派名、题材名、情绪标签和“电影感 / 高级感 / 治愈感 / 青春感”等审美大词拆成当前 Clip 必要的 3—5 个（或更少）可见 / 可听执行项；具体特征足够时可省略导演名，并避免误触发校园、樱花、黑金、霓虹等默认视觉包。风格信息不会压过主体、动作、空间、时间顺序、镜头与状态承接；“短”或“长”本身都不是质量标准。
...
- 常规视频 Prompt 默认不出现音色字段；每个 Clip 的反向提示词固定以禁背景音乐规则开头。
