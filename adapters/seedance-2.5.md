...
**Clip 目标时长只接受整数秒。** 平台参数为 `duration`（单位秒），取值是 [4, 30] 内的整数，或 `-1`（由模型在 [4, 30] 内自选整数秒）；小数秒只出现在 video editing 任务继承源片时长的情形。因此 Clip时长与Clip Plan记值必须是整数秒，不得出现 6.5 秒这类值——它不是可提交参数。该约束只属于目标时长本身：时间线**末阶段的末端边界**因必须等于已确认目标时长而继承整数，**中间阶段边界是提示词文本、不受任何平台参数约束，可以是小数**（如`[0—3.2秒]`），只要求严格递进、无重叠无断档，并按`rules/03_prompt_rules.md`的数值执行价值规则避免无可见收益的过度精度。取证：BytePlus ModelArk《Dreamina Seedance 2.5 tutorial》的 `duration` 参数说明与示例（`"duration": 10` / `5` / `11` / `20` / `30`），取证日期 2026-09-19。
...
**负面提示词的位置**：官方散布规则要求把负面提示词写进提示词正文的具体情节描述，作为自然语言的一部分融入情节并保持句子通顺，不单独成段——字幕控制写在字幕出现的位置，音频控制写在需要控制音频的段落附近；对画面内容的负向控制同样写在正文中。因此内容相关约束的默认位置是它所约束的阶段，末尾反向字段只承担永久无BGM边界、阶段 / 分镜总数兜底句与真正全局且正向锁不死的残余风险（执行口径由`rules/03_prompt_rules.md`的`# Negative Prompt Boundary Rule`与`knowledge/prompt_compilation/state08_projection.md`的`### Positive Specification And Negative Prompt Placement`拥有）。同一文档同时指出：用负向描述控制画面质量与视觉风格，效果不如正向描述直接，不支持的画面负向不必写成清单。取证：Seedance 2.5官方文档「负面提示词散布规则」整理稿截图，取证日期 2026-09-22；**取证缺口——截图未附文档标题、版本号与发布日期，结论句为整理者归纳**，在补齐原始文档标题与日期前本条目标记`REVIEW`。

Timeline 是能力而非固定字段：单动作、简单一镜到底默认省略；多段动作、蒙太奇或连续长镜头内多个节奏节点，可在既有分镜正文中使用与Clip时长一致、严格递进的`0—3秒 / 第3秒`时间戳语义（小数边界同样可用，见上段）。它不新增最终字段、表格或逐帧技术参数。每个时间段必须说明可见事件、摄影机、动作/对白/音效与结束状态；时间戳不精确承诺模型必然逐帧命中。
