...
|---|---|---|
| Character | `workflows/04_character_asset_workflow.md` | `templates/04_character_asset_prompt.md` + 显式外部图像模型的已选最终提示词模板 |
| Environment | `workflows/05_environment_asset_workflow.md` | `templates/05_environment_asset_prompt.md` + 显式外部图像模型的已选最终提示词模板 |
| Prop | `workflows/06_prop_asset_workflow.md` | `templates/06_prop_asset_prompt.md` + 显式外部图像模型的已选最终提示词模板 |
...
- 图像模型Prompt正文：任何新Image Prompt前由`modules/image-model-selection.md`确认当前资产批次模型，未选择时停止，不默认内置Image。选择后由`modules/assets.md`读取该Adapter声明的独立最终提示词模板：Built-in Image为`templates/24_builtin_image_asset_prompt.md`，Midjourney为`templates/14_midjourney_asset_prompt.md`；新模型必须先建立独立Adapter和模板，不能复用现有图像或视频模型正文。
