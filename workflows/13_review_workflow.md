...
□ STATE-08每个Clip的【反向提示词】首个非空内容行无例外逐字为“禁止生成背景音乐、配乐、BGM、主题音乐、氛围音乐，只保留台词、环境声、动作音效和必要的自然声音。”

□ Review未因缺少配乐而自动启动Music模块；如用户另行提交Confirmed Music Package，只单独复核其Cue归属、进入退出、留白、默认纯音乐与视频隔离，不把结果回写STATE-08
...
□ 最终Prompt是否没有只写知识名称、导演名字或模式ID；重要风格标签若保留，首次出现是否在同一风格段得到项目特定、可执行解释；是否没有暴露内部Opportunity Check / Reflection Record

□ 是否完成Prompt Attention / Control Allocation：已由正式角色/环境/道具资产锁定的外观与结构只作最小确认，没有在人物一致性、环境一致性或逐镜正文长篇复述

□ 是否存在同义重复、跨字段机械复述，或互相冲突/优先级不明的机位、运镜、动作、站位、光线与时间顺序指令

□ 重要文学化、情绪化或导演化描述是否至少落到一种人物、环境、光线、摄影机、声音或时间状态的可见/可听执行项；没有只写“宿命感、勇气、压迫、电影感”等孤立标签

□ 风格解释是否只是替换成另一组同样抽象的词；同一标签是否重复解释；解释项是否与当前Clip无关或机械复述Visual Bible；后续Clip是否只在正式Style Source已锁定时只补delta；动作复杂Clip是否让风格说明挤占主体、动作、空间、时间、镜头与Handoff

□ 是否存在可改为正向目标状态却仍大量使用否定词的情况；除固定禁BGM首句外，反向提示词是否只保留少量正向定义仍难锁死的当前高风险项

□ “创业 / 约会 / 学生 / 婚礼 / 医院 / 黑帮 / 赛博朋克 / 日系青春”等高共现大词是否无依据带入默认场景、服装、道具、天气或美术元素；必要标签是否已经由当前Clip真实存在的具体元素承担执行控制

□ 数字是否按视觉控制价值使用；无实际收益的工程级小数、精确轨迹和伪物理参数已经删除或降精度，最终Prompt没有把视频生成模型当作Blender / Unreal式严格物理仿真器

□ 当前Clip主要主体、动作、空间、镜头路径、状态演变和稳定结尾是否比装饰性风格词、器材名与质量形容词更清楚

□ 是否混入其他镜头/Clip的动作、机位、状态或风格残留；只保留合法连续性继承项、当前Clip真实变化与明确Handoff

□ 是否同时堆叠过多导演、美术、摄影、渲染与质量标签，导致风格互相稀释或挤压主体、动作、空间与镜头控制
