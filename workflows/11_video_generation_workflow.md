...
- `knowledge/quality/prompt_scorecard.md`（编译前后检查与评分；`## Discipline Self-Check｜六条执行自检`只提高可见性，`## 联想事故归因与禁用词登记`在带回生成结果或失败返修时读取）
...
5. 交付或生成当前 Clip 的最终 Prompt 时，同轮按`references/asset_package.md`的`## Package Timing And Delivery`与`## Access Precondition`处理生产交付包（入选、命名与位置判据由该文件拥有）：包是**本交付轮的正式收尾交付物**，随该轮自动执行、不需要用户另行下令——门条件与`## Access Precondition`均满足时同轮附上包（或给出包路径与已生成证据）；条件不满足时按其降级阶梯交付清单 / 命名映射并标注`未打包`，或逐项报告缺失类别与所在 Gate。**不得静默略过交付包**，也不得声称已打包而实际未生成。**交付轮的终点就是Prompt本身**：Prompt加包即本轮收尾，此后不追加"是否代为提交 / 用哪个入口 / 是否消耗额度"的授权Checkpoint——该询问既不是本阶段收尾交付物，也不构成Pending Decision。**包内三类工件必须是完整Template版本**：`04_scenes` / `05_shots` / `06_clips`的源工件各自通过`scripts/validate_delivery_artifacts.py`对应`--kind`的完整性校验后才允许入包；未通过时按缺失类别逐项报告并回到该工件自己的owner补全，**不得以摘要、一行一句的清单或"已完成×××"顶替**（构成、入选与降级形态由`references/asset_package.md`拥有，工件形态由各自Template拥有）。打包不是进入本阶段的前置条件（缺包不阻塞Prompt），但交付轮未附包、也未给出降级形态时不得判完成。

6. 本地 Work / Codex 交付轮必须按以下固定顺序执行，不得只写“已打包”：先保存当前 Clip 的完整 Prompt，再运行`scripts/validate_prompt_package.py <prompt-file> --model <selected-model>`；校验通过后立即运行`scripts/build_asset_package.py --project-root <project-root> --check-prompt <prompt-file> --prompt-model <selected-model> --require-prompt`。构建器返回`ok: true`并给出`package_root`与`archive`路径后，才把 Prompt、包目录和 zip 一并交给用户；任一步失败都要在交付收据中报告具体错误并标记`未打包`，不得生成或声称存在正式交付包。普通 Chat / Portable 环境不调用本地构建器，按`references/asset_package.md`的降级阶梯交付清单与命名映射。

# Completion Gate

只改当前 Clip 的 Prompt 映射问题留在 STATE-08；Clip 边界、预算、尾帧或连续性组织问题回 STATE-07；Shot/Blocking 回 STATE-06；事实或资产回对应 owner。全部检查 PASS 后交付或生成，随后主流程收尾：**交付轮在Prompt加包处收尾，不追加外发授权往返**——不插入提交授权询问、提交入口选择（Dreamina网页端 / 方舟API / 其他）、额度或账号确认，也不把"实际提交"写成Pending Decision；直接写回本轮收尾状态。交付轮必须已按`references/asset_package.md`提交包或其降级 / 缺失报告（**不得静默略过交付包**），且附包时包内`04_scenes` / `05_shots` / `06_clips`三类工件均已通过`scripts/validate_delivery_artifacts.py`对应`--kind`的完整性校验（**摘要形态按未交付计，不因文件名与目录结构正确而放过**）；state写回必须显式声明`State Status: COMPLETE`、由`workflows/workflow_map.md`确定的下一Workflow与`Pending Decision`：仍有未交付Clip时下一Workflow保持本阶段、`Pending Decision`写下一个待交付Clip；全部应交付Clip的最终Prompt都交付后主流程收尾，写`Next Workflow: Project Complete / Post`与`Pending Decision: None`。STATE-09是显式调用阶段：交付完成后**不停在等待成片的Review检查点**、不自动进入STATE-09、不主动索要生成结果，也不代判PASS或失败；用户明确要求审核成片或携带具体成片问题时才按其owner进入STATE-09，用户明确接受具体Take时按Review owner的`Accepted Take Canon Writeback`登记。
