...
先从`视觉连续 / Visual Continuity`、`剧情连续 / Narrative Continuity Only`、`主动切场 / 切世界 / Motivated Scene-or-World Change`中选择一个主分类，再映射Handoff。只有视觉连续允许以下第1或第2类正式引用上一尾帧；剧情连续或主动切场 / 切世界必须进入第3类，不得机械引用。

判定结果只允许以下三类Handoff：

1. `Direct Start-Frame Handoff`：同场景、同动作连续，且当前Clip能够从上一尾帧的同一空间、动作阶段与镜头几何直接开始。上一`[Gxx尾帧]`必须正式列入【参考资产】，并在【首帧参考】中声明“直接继承”。
2. `Reference-Only Handoff`：剧情与人物/道具/环境状态连续，但当前Clip因已确认的景别、机位、视角或构图变化不能从同帧构图直接起步。上一`[Gxx尾帧]`可以且通常应正式列入【参考资产】，并在【首帧参考】中声明“仅作连续性参考”，写清兼容的新构图边界。
...
无论采用哪种模式，【首帧参考】都必须把边界落实为可执行首帧：人物位置与左右关系、身体朝向与视线、摄影机起始位置与轴线侧、景别、主体构图与前中后景、环境与光色、关键道具状态、动作起始阶段。Direct模式不得重新摆位或重播动作；Reference-Only模式必须说明有动机的新机位/景别/视角/构图及与上一尾帧兼容的状态；跨场景模式不得把旧场景空间、构图或光线带入新场景。

## End Frame Constraint Rule

【尾帧限制】属于同一Package的前置生成边界，必须在【主风格】之前先锁定当前Clip最终交付帧，并保存为对应`[Gxx尾帧]`。至少描述：
...
`Previous Clip End State → 当前【首帧参考】判定 → Continuous Action → 当前【尾帧限制】 → Next Clip First Frame Reference Decision`
...
- G02及以后是否已先完成空间/动作连续性判定，再决定上一`[Gxx尾帧]`是否正式进入【参考资产】
- 【首帧参考】是否明确Direct、Reference-Only或跨场景No Formal Tail Reference，并具备人物、摄影机、景别、构图、环境、道具、动作与光线状态
- 【尾帧限制】是否保存当前`[Gxx尾帧]`，包含人物/摄影机/道具/环境最终状态，稳定可冻结，并禁止最后1秒开启新复杂动作
...
【参考资产】只允许使用Canonical Character / Environment / Prop / FX Assets、Voice/Audio Reference与合法首尾帧；上一Clip尾帧只有在Direct或Reference-Only判定成立时才作为正式参考资产引用，跨场景时通常不引用；不得使用Storyboard、Detailed Shot Design或Clip Plan截图/渲染图。
...
建立对白、空间声、动作声、静默与跨镜同期声音连续性。默认情况下，音乐与配乐只保留给STATE-09 Editing/Post，不能进入STATE-08的“音效”；只有用户显式要求由Seedance为明确指定的Clip生成背景音乐时才允许例外。
...
1. **Continuity Classification**：三选一锁定`视觉连续 / 剧情连续 / 主动切场或切世界`。只有视觉连续可正式引用上一尾帧并映射Direct / Reference-Only；剧情连续或主动切场必须No Formal Tail Reference，从当前Scene / World-State / Start Boundary重建。
...
要求Seedance生成任何背景音乐。默认音乐属于后期剪辑。

只有用户显式要求由Seedance为某个或全部Clip生成背景音乐时，才允许在用户明确指定的Clip中追加对应音乐指令；音乐不得替代环境底声、同步前景声和声音尾部，未被明确指定的Clip仍执行默认禁令。
...
- 默认背景音乐禁令只进入【反向提示词】，不得挤占“音效”字段；用户显式要求的背景音乐例外只作用于明确指定的Clip
...
- Continuous Handoff时，是否已依据边界自动选择：把前一G段`[Gxx尾帧]`直接作为后一G段起始帧，或仅作为第一顺位连续性参考并重建兼容的景别/机位/视角/构图边界
...
- Direct Start-Frame Handoff的G02及以后，先确认同场景/同动作连续，把上一段`[Gxx尾帧]`正式列入【参考资产】，在【首帧参考】声明直接继承并从该帧逐项继续
- Reference-Only Handoff的G02及以后，把上一段`[Gxx尾帧]`正式列入【参考资产】，在【首帧参考】声明仅作连续性参考，并说明有动机的机位/景别/视角/构图变化与兼容边界
...
`参考资产：`清单：显式列出本Clip实际使用的Canonical Character / Environment / Prop / FX Assets、Voice/Audio Reference与合法首尾帧，并逐项写明用途和锁定约束；已确认资产不得被临时文字描述覆盖。上一Clip尾帧只有在Previous-Clip Continuity Decision判定为Direct或Reference-Only时才正式列入；跨场景时通常不列入，只在编译前核对人物与视觉连续性。

在序列化该清单前，先按Preflight逐分镜World-State删除非当前Clip出场角色、未使用环境/道具/动作图及当前阶段不适用资产；只有视觉连续的Direct / Reference-Only才加入上一尾帧，剧情连续或主动切场 / 切世界不得加入。再按`knowledge/reference_budget.md`以实际图片文件/帧复算：≤7直接保留独立图；8张且无额外帧需求直接保留；9张确认无待加入连续性图片后才保留；>9才整合同类非角色信息，仍超限按优先级裁剪。已有9张且新增上一尾帧时至少释放1位。最终只传递≤9张真实存在且已确认的图片参考；每个核心角色各自独立三视图/角色锁定图不可合并或由动作图替代。

`首帧参考：`写明上一Clip尾帧的引用判定与当前Clip可执行首帧。Direct与Reference-Only必须引用对应`[Gxx尾帧]`；跨场景必须明确上一尾帧不作正式参考资产、仅作连续性核对，并从新场景Confirmed Asset / Scene初始状态重建。无论哪种模式都逐项描述人物位置/朝向/视线、摄影机起始位置、景别、主体构图、环境、道具、动作起始状态与光线状态。

`尾帧限制：`在分镜之前前置锁定当前Clip最终交付帧，保存为`[Gxx尾帧]`，逐项描述人物最终位置/动作/视线/情绪、摄影机最终状态、道具和环境最终状态，以及下一Clip预计如何使用；必须可冻结、可继承，最后1秒不得开启新复杂动作。
...
当前段的尾帧资产名`[Gxx尾帧]`，以及自动判定的下一段用途：直接作为下一段起始帧、仅作为下一段连续性参考、不作正式参考资产仅作连续性核对（跨场景原因），或最终收束。
...
`参考资产：`是否逐项列出当前Clip实际使用的Canonical角色、环境、道具、FX、Voice/Audio Reference与合法首尾帧，并写明用途和锁定约束；是否以已确认资产优先于临时文字描述。缺失、占位或未参与生成约束时不得输出。
...
- Continuity Classification是否在视觉连续、剧情连续、主动切场/切世界中三选一；只有视觉连续正式引用上一尾帧，剧情连续或主动切场没有机械引用。
...
是否每个分镜的“起始状态”明确首帧来源或首帧要求。Continuous Handoff是否直接从上一分镜“镜头结尾状态”或上一Clip`[Gxx尾帧]`继续，且没有重新初始化人物站位、动作阶段、环境、道具、摄影机边界或轴线。
...
是否每个分镜都明确与下一镜/Clip的Boundary Class。叙事性场景切换是否明确选择“实体首帧继承”或“状态基准参考”，或写明不继承、重建原因与保留锚点；是否不存在无说明的连续性丢失。
...
默认情况下，每镜“音效”只含对白、环境声、动作声、呼吸、Foley与剧情内真实声源，不含背景音乐、配乐、BGM、主题音乐、氛围音乐、歌曲、节拍或“无配乐”等音乐说明；若用户显式要求由Seedance生成背景音乐，音乐指令是否只出现在用户明确指定的Clip且没有替代必需的正向制作声音。

每镜“音效”同时具备具体环境底声/有意静默、至少一个同步前景声和声音尾部，不得写“无”“静音”“有效内容”或仅写禁令。

未触发用户显式背景音乐例外时，`反向提示词：`首个非空内容行逐字为“禁止生成背景音乐、配乐、BGM、主题音乐、氛围音乐，只保留台词、环境声、动作音效和必要的自然声音。”，不得省略或改写；触发例外时，是否只豁免用户明确指定的Clip。
...
每个Clip区块均在`主风格：`之前完整输出`首帧参考：`与`尾帧限制：`，无条件输出`音色特征：`，并在末尾输出`反向提示词：`；尾帧保存为对应`[Gxx尾帧]`。

Continuous Handoff的后一Clip先判定Direct或Reference-Only，再在`参考资产：`、`首帧参考：`和首镜“起始状态”中明确引用上一段尾帧；跨场景Motivated Discontinuity不把上一尾帧列为正式参考资产，只用于人物/视觉连续性核对，并明确重建原因。
...
只有用户显式要求由Seedance为特定Clip生成背景音乐时，才为每个明确指定的Clip追加：

--background-music-clip CLIP-xxx
