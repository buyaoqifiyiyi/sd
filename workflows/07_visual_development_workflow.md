...
Production Setup已经确认项目风格基线`Project Style Baseline`。
...
在风格检索和参数定义前先读取与Production-Locked剧本绑定的Writer Intent Packet（owner：`knowledge/writer/screenplay_development.md`）中的Theme / Story Arc、Motif / Symbol Function、Information Architecture与Setup / Payoff obligations，再读取Project / Scene Director Intent（owner：`knowledge/director_decision_layer.md`，经STATE-01 Writer → Director Handoff绑定）建立项目级Visual Arc。Writer决定这些元素为何存在、何时建立或回收；Director把它们转译为Visual Dramaturgy：
...
- 先在既有Visual Direction / Project Bible中确定`Visual Grammar Baseline`（稳定的摄影/材质质感、可用色谱与强调色条件、真实光源、空间气质、构图/景深与摄影机介入倾向），再为每个戏剧阶段仅记录有事实依据的Scene Delta；统一不等于每场同色、同景别、同构图或同运镜。已确认的`Project Style Baseline`是它的主风格标签与项目内含义来源：本阶段在其上展开体系，不改写基线标签；需要改变整体风格时按`# Revision Rule`处理并回写基线
...
其索引命中的对应视觉风格 / 导演原子文件；未命中不读，不得整树通读该目录。
...
knowledge/performance/index.md（先据index定位，只读建立项目级表演语法所需的原子文件；不整树通读）
...
knowledge/sound_language/index.md（先据index定位，只读建立项目级声音原则所需的原子文件；不整树通读）
...
STATE-01 `Production Setup Gate`已把`Project Style Baseline`写入本区块；STATE-04在其上完成正式Visual Direction与`Aesthetic Decision Lock`并更新同一区块，不新增竞争字段，也不改写基线标签。
