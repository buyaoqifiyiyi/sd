...
- `音效 / BGM`必须以清楚的独立子项分别记录`环境声`、`同步音效 / Foley / 呼吸 / 剧情内声源`、`声音尾部`与`后期BGM规划`。默认BGM只属于后期剪辑计划，不进入STATE-08音效；只有用户明确要求Seedance生成BGM时，才由STATE-08按既有例外规则处理。
...
音乐与配乐只在后期剪辑阶段规划，不进入STATE-08的音效设计。Shot Design为视频生成准备的声音只包含对白、环境声、动作声、呼吸、Foley与剧情内声源。
...
□ 每镜`音效/BGM`均区分可生成同期声与仅供后期的BGM规划；默认后期BGM没有进入STATE-08音效
