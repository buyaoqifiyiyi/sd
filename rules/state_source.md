...
在解析任何本地Root或Portable State之前，先判断当前请求是否需要持久化项目资料：

- 新项目、从创意/剧本开始的普通制作请求，且当前会话没有已验证项目上下文时：直接建立会话内最小STATE-00；不得扫描磁盘寻找历史项目，不得按项目名/素材相似度匹配，也不得为此查询Skill Experience。
- 当前会话已有已验证项目上下文：优先继续该上下文，不重复枚举磁盘上的其他项目。
- 用户明确说恢复/查找/保存/归档项目，给出Project ID、Project Root或明确指向某个已有项目时：才使用用户给出的路径或Project ID、当前可访问的项目目录或Portable State；不得以名称、题材、素材或“类似项目”猜测匹配。

本Skill不维护项目登记表：不存在也不得建立任何项目索引、清单或注册文件；找回项目只依赖用户给出的路径/Project ID、可访问的项目目录与Portable State。
