...
`templates/08_shot_design_prompt.md`的默认用户可见形态是5列表；用户明确要求“完整版专业分镜”（或等价的“给我完整版 / 十八列 / 完整字段分镜”）时，交付形态改为十八列完整记录。它是**合法用户可见交付**，不属于本Rule常规禁止逐项展示的内部生产数据；`AI制作备注`仍只写边界与执行信息，内部Camera Language Decision、Execution Risk、Director Decision Notes与Knowledge Reflection不因放宽形态而成为可见字段或附录。

这条路径历史上失败过一次，失败形态是“看起来像完整版、实际不是”：用户要完整版，代理给出的是默认5列表加两批十八列，其余批次改用压缩摘要表，而**校验器全部通过**。因此完整版是**文件交付**，不是聊天里的展开：压缩摘要表可以辅助阅读，但不能代替完整版本身。

- **一次一个文件，一次全部Shot**：完整版必须保存为`05_shots/06_detailed_shot_design.md`（历史命名`08_detailed_shot_design.md`亦可），与默认五列形态同源、不改变任何已确认镜头；该文件必须一次包含**全部已确认Shot**，按`SHOT-001`起的原序、连续、不重复。分批只允许出现在“批次标题 + 完整十八列表头”的同文件结构里，**不得**跨轮拆分文件、不得以“下一轮继续”留下缺镜，也不得用压缩摘要表代替任一镜的十八列。
- **判别句**：完整版的“完整”指**镜**，不是指**列**。十八列但只有前两批＝不完整；30镜齐全但只有五列＝不是完整版。两者都必须报为未完成，而不是“已交付完整版”。
- **交付回执**：显示完整版时必须逐批给出已交付镜号区间与`Last Fully Delivered Shot`／`Next Undelivered Shot`；最后一批给出`Timeline And Coverage Summary`。全部Shot交付完毕前，状态写`待交付`，不得写“完整版已交付”。
- **确定性校验**：保存文件后必须运行`scripts/validate_delivery_artifacts.py <artifact-file> --kind shot-design-full`；该形态断言单一文件、十八列表头、每格非空、镜号自`SHOT-001`连续、正文声明的总镜数与实际行数一致、时间码可复算且不断档。未通过不得交付；通过也不替代语义检查。
- **回退路径**：`--kind shot-design`仍只认默认五列表或十八列完整表，继续服务打包路径，不因本条改变；用户没有要求完整版时不得主动产出十八列文件。
...
- 模型适用时长账本（2.0为4—15秒；2.5为4—30秒，16—30秒须严格预检PASS，实际秒数由用户选择）、模型复杂度、风险降级、尾帧用途与下一Clip Handoff



禁止：

把Storyboard图片、分镜板、漫画格、接触表、拼图、Scene Top-down Blocking Map、Look Frame试片帧（`templates/25_look_frame_prompt.md`）或多画面材料用作Clip/STATE-08输入；也禁止Clip Production替代Detailed Shot Design。经Before-Single-Clip-Prompt Gate确认的单Clip `REF-SKETCH`只作为Visual Blocking Reference严格例外，不开放其他线稿或Planning材料。
...
- 禁止Storyboard图片、分镜板、拼图、Look Frame试片帧、Scene Top-down Blocking Map或多画面参考；不得把未验证草图混入上述例外
