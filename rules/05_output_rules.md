...
- 每个Shot完整记录：镜号、TC IN、TC OUT、时长(s)、景别、焦段、场景/美术、画面内容/构图、人物动作、摄影机/镜头、摄影参数、镜头调度、光线/色彩、画面特效/转场、台词/旁白/口播、音效/BGM、AI制作备注、素材/资产
...
内部Camera Language Decision、Execution Risk、Director Decision Notes与Knowledge Reflection不得成为正式用户可见字段或附录。`台词/旁白/口播`是泛化字段，不得退回固定“女声口播”。`音效/BGM`中的BGM默认只作后期规划，不自动进入STATE-08正向音效。

### Per-Shot Structural Parity Hard Gate

- 单镜输出与批量输出必须逐Shot使用完全相同的十八个Template字段、相同顺序与相同完成标准；批量只改变Shot数量，不改变单镜Schema或内容密度。
- 不得因篇幅、总镜数、上下文长度或输出上限删字段、合并既定字段、缩写/改名字段、使用简化表头、把多镜内容合写为摘要，或只给首镜/示例镜完整结构。
- 每个Shot的`画面内容 / 构图`必须分别明确画面描述、构图与人物位置关系；`人物动作`必须分别明确人物情绪、动作重点与完整动作链；`音效 / BGM`必须分别明确环境声、同步音效、声音尾部与后期BGM规划；`AI制作备注`必须分别明确角色一致性、环境一致性、道具一致性与生成风险/控制项。
