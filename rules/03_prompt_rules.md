...
STATE-08格式必须逐Clip严格服从`templates/10_video_prompt.md`固定契约：每个Clip结构完全相同，所有字段按顺序完整保留；不得因批量或篇幅压缩、合并、共享、删减或改名字段。内容过长时只在完整Clip之间自动分批。`参考资产：`、`首帧参考：`、`尾帧限制：`、`音色特征：`无条件必填；每个分镜完整重复十个固定字段，不得增加“与下一镜衔接”或其他字段。任何旧格式冲突以Template为最高优先级，输出前必须逐Clip执行字段完整性检查。
...
由Seedance生成任何背景音乐。默认音乐设计只属于STATE-09 Editing/Post，不进入STATE-08声音字段。

默认情况下，最终【反向提示词】首个非空内容行必须逐字写“禁止生成背景音乐、配乐、BGM、主题音乐、氛围音乐，只保留台词、环境声、动作音效和必要的自然声音。”，不得省略、改写或移到其他限制之后。

唯一例外是用户显式要求由Seedance为某个或全部Clip生成背景音乐。例外只作用于用户明确指定的Clip：这些Clip可省略上述默认行，并可在现有“音效”字段中追加用户明确要求的音乐生成指令，但不得用音乐替代环境底声、同步前景声与声音尾部；未被明确指定的Clip仍执行默认规则。不得从题材、情绪、视觉风格、参考作品或后期配乐计划推定例外。
...
“禁止生成背景音乐、配乐、BGM、主题音乐、氛围音乐，只保留台词、环境声、动作音效和必要的自然声音。”是STATE-08默认平台边界，不受“只选项目高风险项”规则影响，必须作为【反向提示词】首个非空内容行保留；只有用户显式要求由Seedance为当前Clip生成背景音乐时才可省略，且例外不得扩展到未明确指定的Clip。
...
未触发用户显式背景音乐例外时，“音效”是否只包含对白、环境声、动作声、呼吸、Foley和剧情内真实声源，且没有任何音乐说明；触发例外时，音乐是否只出现在用户明确指定的Clip且没有替代必需的正向制作声音。

未触发用户显式背景音乐例外时，`反向提示词：`首个非空内容行是否逐字为“禁止生成背景音乐、配乐、BGM、主题音乐、氛围音乐，只保留台词、环境声、动作音效和必要的自然声音。”；触发例外时，是否只豁免用户明确指定的Clip。
