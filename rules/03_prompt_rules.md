...
输出分辨率与宽高比不属于本节参数：它们是用户在生成时自行选择的平台参数，不写进视频Prompt，也不得另设字段声明。
...
`主风格 / 人物一致性 / 环境一致性 / 参考区 / 各分镜字段`等位于其前的正文必须以正向、可执行、可观察的目标状态为主。负向控制的**默认位置是它所约束的内容所属的阶段**：与某个具体动作、空间关系、道具状态、表演、光线、声音或字幕相关的内容约束，写在该内容所属的字段内，作为情节描述的自然语言一部分——字幕控制在字幕出现的位置写入，音频控制在需要控制音频的段落附近写入。只有真正跨阶段、不归属任何单一阶段的**通用**负向清单（通用的“禁止…… / 不要…… / 避免……”限制与生成错误规避项）才统一归并到末尾唯一的`反向提示词：`。内容相关约束不得被抽离所属阶段搬到末尾，那会切断指代并削弱执行精度。

执行`Negative Constraint → Positive Target State`：优先描述画面应该是什么，而不是反复提及不应出现的概念。例如“不要下雨”优先改为“晴天，空气清透，阳光照亮地面”；“不要夸张表情”优先改为“表情克制，嘴角和眉眼只有轻微变化”。“禁止夸张微笑、甜宠式表演、广告摆拍、MV慢动作与炫技运镜”应优先转成“表演克制含蓄，镜头调度简洁自然，优先服务人物关系与情绪留白”等正向执行要求；仍属当前Clip高风险且正向状态难以完全锁定的残余错误，才压缩后进入末尾反向提示词。**保持在所属阶段的内联内容约束同样先执行本改写**：能写成目标状态就不留否定形式，只有正向句仍锁不住边界时，才用最短否定句留在该阶段。如果正向状态已经充分锁定，不再把同一要求重复塞进反向提示词。

内联内容约束的写法：优先写成正向持续状态，例如`左手持续握住伞柄，整个动作链保持左手持有`；只有正向句仍无法明确边界时才保留最短局部否定。它只约束该局部，不得扩展成通用负向清单，也不得把同一通用限制复制到多个分镜；跨镜通用部分仍归入末尾唯一反向提示词。逐镜的结尾稳定、禁止提前动作和合法衔接属于此类内容相关边界，必须保持最小、贴近受控动作并服务连续性，不能被机械移走导致执行不清。


末尾`反向提示词：`只汇总**真正全局、跨阶段、且难以通过正文正向状态锁定**的残余错误（固定平台边界与阶段 / 分镜总数兜底句不在此限）：它只留不下放到任何单一阶段的东西。删除低价值、抽象化、与正文重复、同义重复或与当前Clip无关的项；历史事故物、其他Clip状态和未来情节默认删除，同类项合并压缩为当前Clip失败类别，且不得在篇幅或语义权重上压过主体、动作、空间、摄影机、时间顺序和状态承接。具体Negative Compression算法只由`knowledge/prompt_compilation/state08_projection.md`拥有，本Rule不复制类别表或阈值。

“禁止生成背景音乐、配乐、BGM、主题音乐、氛围音乐，只保留台词、环境声、动作音效和必要的自然声音。”是STATE-08永久平台边界，不受“只选项目高风险项”规则影响，必须作为【反向提示词】首个非空内容行保留，不存在任何省略或Clip例外。它与阶段 / 分镜总数兜底句是末尾字段的两项永久内容。
...
- 把每镜边界合同替换成一长串负面词
- 在每个镜头重复相同负面限制
- 在`主风格`、一致性字段或逐镜正文散布**通用**负向清单（写在内容所属阶段的最小内容约束不在此列）
- 把内容相关的约束抽离所属阶段移入末尾反向段，导致分镜指代不清
...
反向提示词的最终位置和字段名称仍由当前阶段Template定义。内容相关负向写在所属阶段这一默认口径的能力依据与取证状态见`adapters/seedance-2.5.md`。
...
分镜或阶段总数声明是否落在末尾反向约束字段或`时间线：`，并与实际数量完全一致：Seedance 2.0在末尾`反向提示词：`声明本Clip严格只有N个分镜；MiniMax H3在`画面过程说明`首行声明段数并由末尾`反向提示词：`保留同义兜底；Seedance 2.5由`时间线：`首行声明阶段总数并由`全局限制与反向提示词：`保留同义兜底。缺失、虚报或与实际数量不一致均为失败，且不得为此新增任何字段。


输出分辨率与宽高比是否已经排除在Prompt之外：它们是用户在生成时自行选择的平台参数，不得作为字段或正文内容出现在最终视频Prompt中。
