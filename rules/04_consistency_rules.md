...
1. **视觉连续 ≠ 剧情连续；尾帧需求 ≠ 尾帧当前可用性。** 当前Clip必须先在`视觉连续 / 剧情连续 / 主动切场或切世界`中三选一，并根据当前Clip首帧是否需要严格视觉承接，在既有Previous-Clip Continuity Decision中标记`Tail Frame Required = YES / NO`；不得用“系统当前是否已有尾帧图”反向决定需求。Direct / Reference-Only需要精确继承上一Clip可见状态时必须为`YES`。`YES`但尾帧尚未提供时，STATE-07 / STATE-08必须主动请求用户从上一Clip最终成片中手动截取最终有效尾帧并上传，资产状态明确为“待用户提供/待上传”；允许形成Prompt草案，但不得形成最终可执行版Prompt。剧情连续、主动切场或其他不要求严格视觉承接的边界标记为`NO`，可仅以文字状态承接或建立新首帧。
...
进入STATE-08后，边界信息不得因“一个Clip包含多个分镜”而被隐式处理。同一Clip内每个分镜都必须保留起始、结尾和下一镜衔接；每个Confirmed Clip成为独立G生成段，在【主风格】之前输出【首帧参考】与定义自己新结束状态的【尾帧限制】，并输出独立反向提示词。实际生成并确认的尾帧资产统一命名为`REF-TAIL-XX｜CLIP-XX尾帧参考`，其中`XX`沿用来源Clip编号。跨Clip必须先根据当前Clip Start Requirement判定并标记`Tail Frame Required = YES / NO`，再检查实际尾帧资产；资产是否已存在只决定“可正式引用 / 待用户提供”，不得反向改变是否需要严格承接。跨场景或其他非严格承接通常标记`NO`，不正式引用旧尾帧，只作必要的人物与视觉连续性核对。

## Cross-Clip Tail-Frame Carryover

严格承接上一Clip结尾时，必须同时检查文字承接与视觉资产承接，并沿用现有Previous-Clip Continuity Decision、`参考资产：`、`首帧参考：`、`尾帧限制：`和逐镜起止状态，不新增最终Prompt字段或平行Schema。`Tail Frame Required`是既有连续性判定中的内部标记，不改变Seedance Prompt结构。

- 连续动作跨Clip延续、姿态需精确承接、站位/朝向/人物间距离重要、道具位置或使用状态重要、起始构图需高度一致、同场景连续拍摄，或模型容易发生位置跳变、动作重置、左右翻转或空间重构时，必须标记`Tail Frame Required = YES`；判定只取决于当前Clip是否需要严格视觉承接，不取决于系统是否已经拥有图片。
- `Tail Frame Required = YES`且上一Clip尾帧尚未上传时，STATE-07 / STATE-08必须主动提示用户：从上一Clip最终成片中手动截取最终有效尾帧，并作为当前Clip参考资产上传。此时不得创建`REF-TAIL`名称占位、伪造路径、假设已上传或把文字End State登记为图片资产；可以输出Prompt草案，但必须将该尾帧明确标记为“待用户提供/待上传”，最终可执行版Prompt必须等待尾帧提供后形成。
- 尾帧上传、可访问并确认可用后，统一命名为`REF-TAIL-XX｜CLIP-XX尾帧参考`，必须加入当前Clip`参考资产：`。当前Clip的`首帧参考：`必须逐字包含：`以 REF-TAIL-XX｜CLIP-XX尾帧参考 为直接承接依据起镜。`，随后继续锁定人物姿态、人物位置、朝向、人物间距离、构图、机位关系、环境状态、光线、天气、道具状态与情绪状态。
- 当前Clip不需要严格视觉承接，例如换场、明显时间跳跃、构图无需连续或画面独立重建时，标记`Tail Frame Required = NO`，不得要求用户截图；可只用文字End State核对仍有效状态，或从当前Scene、World-State、Start Boundary与Canonical基础资产建立新首帧。
...
STATE-08中必须先比较上一Clip End State与下一Clip Start Requirement，先确定Direct / Reference-Only是否构成严格视觉承接并在既有判定中标记`Tail Frame Required = YES / NO`，再核验对应`REF-TAIL-XX｜CLIP-XX尾帧参考`是否实际存在、可访问且已确认可用。`YES`时不得因图片暂缺降级为纯文字最终承接：必须主动请求用户从上一Clip最终成片手动截取最终有效尾帧并上传，在提供前只允许明确标为“待用户提供/待上传”的Prompt草案，不得形成最终可执行版Prompt。上传后把尾帧正式列入下一段【参考资产】；Direct直接继承，Reference-Only仅作第一顺位连续性锚点并建立兼容的新机位/景别/构图。仅剧情因果连续而画面独立建立、换场、时间跳跃或构图无需连续时标记`NO`，不得要求截图，可文字核对或重建首帧。下一段起始状态不得与合法尾帧或已记录End State冲突，也不得重播已完成动作。
...
STATE-08中跨场景的下一G段通常不得把上一`REF-TAIL-XX｜CLIP-XX尾帧参考`列入【参考资产】或作为生成输入；【首帧参考】必须明确该尾帧只作人物身份与视觉连续性核对、写明经确认的重建原因，并只保留剧情授权的身份、服装、道具后果、情绪或主题锚点。若该尾帧资产本就不存在，只核对文字End State，不得虚构资产名。
...
- 【参考资产】显式列出当前Clip实际采用的已确认角色、环境、道具、FX、声音Reference与合法首尾帧，并写明用途和不可改动约束；已确认资产优先于临时文字描述；先在既有连续性判定中标记`Tail Frame Required = YES / NO`，`YES`且实际尾帧图存在、可访问、已确认时按统一`REF-TAIL`名称正式引用，尚未提供时不得虚构且最终可执行Prompt必须暂停
