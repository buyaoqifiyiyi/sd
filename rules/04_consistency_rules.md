...
1. **视觉连续 ≠ 剧情连续。** 当前Clip必须先在`视觉连续 / 剧情连续 / 主动切场或切世界`中三选一。视觉连续必须检查上一Clip是否有实际可用尾帧图：存在时强制正式引用；不存在时不得虚构资产，只能以文字承接上一Clip End State并继续使用Canonical基础资产。剧情连续或主动切场不得机械引用，必须从当前Scene、World-State与Start Boundary重建。
...
进入STATE-08后，边界信息不得因“一个Clip包含多个分镜”而被隐式处理。同一Clip内每个分镜都必须保留起始、结尾和下一镜衔接；每个Confirmed Clip成为独立G生成段，在【主风格】之前输出【首帧参考】与定义自己新结束状态的【尾帧限制】，并输出独立反向提示词。实际生成并确认的尾帧资产统一命名为`REF-TAIL-XX｜CLIP-XX尾帧参考`，其中`XX`沿用来源Clip编号。跨Clip先判断空间/动作连续性并检查实际尾帧资产：Direct或Reference-Only且资产实际可用时，由下一段【参考资产】正式引用；资产不存在时不得虚构，只作文字承接；跨场景时通常不正式引用，只作人物与视觉连续性核对。

## Cross-Clip Tail-Frame Carryover

严格承接上一Clip结尾时，必须同时检查文字承接与视觉资产承接，并沿用现有`参考资产：`、`首帧参考：`、`尾帧限制：`和逐镜起止状态，不新增字段或平行Schema。

- 若上一Clip存在实际可用的最终尾帧图、定格图或经确认的尾帧截图，必须将其作为当前Clip参考资产之一，统一命名为`REF-TAIL-XX｜CLIP-XX尾帧参考`。当前Clip的`首帧参考：`必须逐字包含：`以 REF-TAIL-XX｜CLIP-XX尾帧参考 为直接承接依据起镜。`，随后继续锁定人物姿态、人物位置、朝向、距离、构图、机位关系、环境状态、光线、天气、道具状态与情绪状态。
- 若上一Clip没有实际可用尾帧图，不得创建名称占位、伪造路径、假设已上传或把文字End State登记为图片资产。`参考资产：`不得列入该尾帧；`首帧参考：`只用文字完整描述上一Clip结尾状态，并依靠Active Canonical角色、环境与道具资产维持连续性。
- 连续动作跨Clip延续、姿态需精确承接、站位/朝向/距离重要、道具位置或使用状态重要、起始构图需高度一致、同场景连续拍摄，或模型容易发生位置跳变、动作重置、左右翻转或空间重构时，优先取得并使用实际尾帧参考。
...
`上一Clip尾帧 → 当前Clip参考资产 → 当前Clip首帧参考 → 当前Clip生成 → 当前Clip尾帧限制 → 下一Clip尾帧承接`
...
STATE-08中必须先比较上一Clip End State与下一Clip Start Requirement，并核验对应`REF-TAIL-XX｜CLIP-XX尾帧参考`是否实际存在、可访问且已确认可用。同场景、同动作连续且尾帧资产可用时，把该尾帧正式列入下一段【参考资产】并直接继承；可见人物/空间/道具状态仍属视觉连续、但机位/景别/构图有动机变化时正式引用并仅作连续性参考。若没有实际尾帧图，仍保持Continuous Handoff语义，但不得虚构参考资产，只在【首帧参考】与首镜“起始状态”中用文字承接上一Clip End State。仅剧情因果连续而画面独立建立时不属于Continuous Handoff，不得机械引用。下一段起始状态不得与合法尾帧或已记录End State冲突，也不得重播已完成动作。
...
- 【参考资产】显式列出当前Clip实际采用的已确认角色、环境、道具、FX、声音Reference与合法首尾帧，并写明用途和不可改动约束；已确认资产优先于临时文字描述；上一Clip尾帧只在Direct或Reference-Only成立且实际尾帧图存在、可访问、已确认时按统一`REF-TAIL`名称正式引用，无图时不得虚构
