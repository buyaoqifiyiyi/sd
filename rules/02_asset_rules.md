...
**路由覆盖不变量**：选定参考集之前必须先列出当前Clip的风险清单（身份 / 服装 / 空间结构与方位 / 轴线与反向背景 / 道具造型与可识别状态 / 特效 / 光线与场景状态），并逐项标记 `已路由到<真实输入>` 或 `文字锁`——**二者必居其一，不得静默留空**：

- 标为`已路由`的，该输入必须真实存在、已确认，并在`参考资产`里列出；标为`文字锁`的，必须写进对应文字字段（`主风格 / 环境一致性 / 起始状态 / 道具状态 / 首尾帧限制`）并说明为何没有可用或不需要视觉输入。
- 同一空间有多张Active Canonical View时：Clip存在反向背景、轴线两侧、纵横距离或门窗/地标朝向风险 → 至少覆盖Master（`ENV-01`）与Reverse或Lateral中的一张。**只列Master却存在上述风险，判定为路由不完整**——这正是"包里明明有图、成片还是穿帮"的成因：图在包里，但没有被路由进Prompt。**人物或道具沿墙、玻璃、窗、门框、栏杆、幕墙行进或跨越平面时，同一要求适用**：至少要有能建立该平面位置的反向或侧向View，否则模型无从判断平面在哪一侧。
- **固定平面与反射是独立风险**：命中时除View覆盖外，还必须在`环境一致性 / 起始状态 / 画面描述 / 镜头结尾状态`里锁定——（a）人物与道具在结构的哪一侧、是否允许跨越；（b）反射是否表现、以何形式（正常镜像 / 只作前景遮挡 / 明确不表现）；（c）不得出现第二个副本。**深度顺序不得反转**：窗框、立柱、墙垛、栏杆这类前景结构只遮挡主体，主体不得穿过结构，也不得出现"结构遮住主体一部分、主体又遮住结构另一部分"的互穿；主体与结构在同一帧里必须只有一个前后关系。
- **服装与道具的状态是独立风险**（敞/合、亮/灭、干/湿、完整/残破），不能用一个身份图或单张道具图代替；命中时必须有对应的Costume / State Canonical Reference，或在文字锁里逐镜锁定起止状态。
- 覆盖清单属于内部生产记录（Reference Selection / Routing），**不新增Prompt字段**；它只通过既有`参考素材职责与优先级`与参考条目体现。
