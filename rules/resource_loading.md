...
本节定义**运行期的少读纪律**。Skill自身体量的阈值与Size Ledger由`references/context_budget.md`拥有；本节不重复其数值，只规定运行时如何避免无效读取。文件长度本身不构成读取理由，文件越短也不成为跳读owner的借口。

1. **优先读owner**：先读当前事项的唯一owner文件，不从多个文件拼装同一事实；其他位置只用于路由、引用或不变量核对。
2. **长文件按章节读**：超过Target阈值的文件，只读与当前事项直接相关的章节，不整文件通读，不为“保险”预读全库。
